...
## 19 September checkpoint: materials explain their use

Inventory Examine on fire clay, silver sand or ward oil now shows a readable description over the inventory: what one measure repairs, whom to speak to, the required guide and whether it is carried, and that the material is spent while the guide is kept. Fire clay also names Morrowfast's quiet bell and its free alternative. Every claim is pinned against the code that performs it. Native proof examines fire clay earned from real deliveries and spends it on the quiet bell (R129, 81/81); full R130 **15,010/15,010**, zero compiler errors. Silver sand and ward oil are EditMode-covered only; the text names no destination settlement. See [plan and acceptance](RELEASE-MATERIAL-GUIDANCE-PLAN.md). No new mechanic, stock, save field or asset.

## 19 September checkpoint: the first middle-game chain

The Stillleaf Archive is a complete, ordinarily obtainable middle-game chain: a Recension Searcher at Quillhold hires the player and gives the keeper's last words; a Pale Curation Indexer at the Salt-Vault finds the keeper's file by those words and names Curation's request (agree, bargain or refuse) before releasing the vault key from a locked salt file; the key opens Stillleaf's sealed library by the ordinary bump; the register inside is decided once — delivered to Quillhold, filed beside its keeper, or resealed in place — with standing following what was promised and the other party told the truth for free. It survives the wrong order (theft of the key, the slate giving the words), a lost register, dead participants and save/load; four gaps found by the adversarial sweep were fixed. EditMode **15,109/15,109**; native journey **102/102 checks over 1,414 native steps** with 33 captures inspected; zero compiler errors. Native proof covers the delivery outcome only; filing, resealing, refusal, theft and loss are EditMode-covered. See [MIDGAME-STILLLEAF-ARCHIVE](MIDGAME-STILLLEAF-ARCHIVE.md). Roadmap step 3 is met for one chain; the god-contact/ending spine (§15) remains unbuilt.

## 19 September checkpoint: the closure-ledger and the first enacted ending

Roadmap step 4 is met for one enacted route. Every act the player takes on — village errands, the Morrowfast and Stillleaf chains, regional requests — now sits on a closure-ledger that ends in a sentence or stays open: carried through (closed), ended by a spoken no to the giver's face (refused), or dropped in silence (open, never written off). The journal shows the ledger's line, REFUSED beside COMPLETED, and UNSPOKEN acts with where to end each; every quest giver can be told no; a giver who is gone makes an act lost, and [R] renounces lost acts aloud. At the Felling-Site's empty seventh position the player is struck (vessel-path Renewal, its crack stated) or refuses aloud and names the world (practice-path Renewal, gated on a clean ledger, which names what is open instead); either persists, announces its epilogue with its permanent costs, and ends the seventh's exposure. No ending is called best. EditMode **15,159/15,159**; the adversarial sweep found one gap (out-of-range place names), fixed; native journey **113/113 checks, validated** over 1,460 native steps with 36 captures inspected, enacting the practice path. Bounds: the vessel path and the open-ledger refusal are EditMode-covered, not captured; faction consequences are stated, not enacted by NPCs; the *sari… sari…* has no ambient hook; Consume and Preserve are advertised routes, not built; village quest definitions still lack display names. See [ENDING-SPINE](ENDING-SPINE.md).

## 19 September checkpoint: the other advertised routes

Roadmap step 4's second sentence is met for the routes the Palimpsest's page advertises: the Root is a place on the map (the stump's crown, two cells north of the Felling-Site: a cleft with a way down and loose tepuibone, and the taproot's face below), and beside the face the player can let the Choir in (Gathered: nothing is ever lost, no one remains) with a cutting any choir tendril gives after saying its cost, or seal the Root (Kept: nothing heals, the Six survive as themselves) with tepuibone, memory-marble from the Recension Searcher and mute-stone from the Curation Indexer, each of whom says the margin first. Each is once and for all with the two Renewals at the seventh position; taking a stone or the cutting takes on an act on the closure-ledger that any giver ends aloud on request and the enactment closes; a giver gone makes it lost and renounceable; no ending is called best. EditMode **15,223 tests, 15,223 passed**; the adversarial sweep found one gap (an impostor face enacted), fixed; native journeys **119/119 and 119/119 checks, both validated** over 1,465 and 1,472 native steps, enacting Kept in one and reading both offers in the other. Bounds: Gathered is not native-captured; Selen and the Cathedral name-scene, the Reader-Salted reconciliation and Dohren's gate are not built; consequences are stated in the epilogues, not enacted by NPCs. See [ENDING-ROUTES](ENDING-ROUTES.md).
