...
| Commit | What |
|---|---|
| `feat/lore-absurd-liquids` | **LA — 5 absurd-property lore-liquids + 5 engine extensions + 5 bench audit dimensions** — five qualitatively-NEW combat-mechanic coats (not just another stat/resistance dial): `veined-pulse-mycelium` (total Electric immunity — forces Amount=0, distinct from resistance which scales — §L5 Branchwork), `choir-mirror-mucilage` (50% damage reflect; null-source on reflected hit = cycle-breaker for two-mirror infinite-loop scenario — §L4 Choir bidirectional digestion), `felling-counter-resin` (HP rewind: snapshot at OnTurnStart/Apply, restore at OnTurnEnd BEFORE dry-down — only undoes NET damage, refuses to resurrect — §L3 Antikythera time-tech), `pebble-sundew-dew` (knockback 1 cell opposite attacker via Zone.MoveEntity, blocked-cell graceful no-op — §L6 threshold-greeting), `held-breath-lacquer` (PreventDeath + BlockAction — permanent undying-and-pacifist, distinct from Memory-Bath's one-shot — §L8 Apatheia). 5 new fields on LiquidDefinition (`ImmuneElement`, `ReflectPercent`, `HpRewindOnTurnEnd`, `KnockbackOnHit`, `PreventDeath`/`BlockAction`); OnBeforeTakeDamage gates re-ordered (immunity → PreventDeath → DeathAnchor → element); OnTakeDamage refactored into TryReflect/TryKnockback dispatcher; AllowAction override; RewindSnapshotHp public field for save round-trip. 5 new bench probes (ImmunityAudit/ReflectAudit/RewindAudit/KnockbackAudit/UndyingAudit) on a fresh `p.y-3` row; ActiveZone pinned in Apply() so SettlementRuntime-resolved zone works for reflect/knockback. ~38 unit tests; live runId-scoped matrix 100/100 + 5/5 LA probes exact: veined-pulse Electric nullified=true (dealt=0); choir-mirror actualReflect=50 (exact 50% of 100); felling-counter wounded 4000→3800→rewound to 4000; pebble-sundew shoved from (45,9) to (45,10) opposite north attacker; held-breath survived=true blocksAction=true. Methodology playbook gained nothing new (LX Rule 8 + LB cycle-breaker patterns both reused cleanly). |
| `feat/lore-buff-liquids` | **LB — 5 positive lore-liquids + 3 engine extensions + 3 bench audit dimensions** — tepui-thread buff coats: `tepuibone-slurry` (+6 AV/+4 Tough/+25 to each elemental res — pure JSON), `convalessence` (+4 HP/turn via signed PerTurnDamage), `lantern-beetle-ichor` (LightSourcePart attach r=6 — respects held lanterns), `memory-bath` (50% Max killing-blow anchor, one-shot via OnBeforeTakeDamage interception), `bower-resin-amber` (+3 DV/+2 AV; rep-multiplier ⚪ deferred). 3 new C# extensions wired through the LiquidCoveredEffect engine (signed PerTurnDamage / LightSourcePart attach with held-lantern guard / killing-blow interception with AnchorConsumed one-shot). 3 new bench probes (TickAudit/LightAudit/DeathAnchorAudit) extend RunMatrixAudit to verify the non-damage-matrix mechanics — runId-scoped (Rule 8) + direct execute_code cross-check (Rule 8 corollary) both clean. Live verification: 20 dummies × 4 elements = 80/80 matrix cells, 0 skipped; tepuibone ×0.75 in every column; convalessence Δ=+4 heal/turn; lantern radius=6 color=&Y; memory-bath triggered=True restoredTo=2000. ~17 new tests, methodology playbook gained nothing new (all rules from LX/LL caught issues already). |
