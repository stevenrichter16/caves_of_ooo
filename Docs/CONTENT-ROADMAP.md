...
- 💡 **v2 expansion (Tier 4).** Tier 2 deferred passives need new combat hooks: Expertise (+to-hit modifier event), Hammer (Broken effect), ShatteringBlows (ShatterArmor effect), Backswing (on-miss event), Hobble (Hobbled/move-slow), Rejoinder (DefenderAfterAttackMissed event), auto-Decapitate/Dismember on penetration. Tier 3 needs new feature systems: ActivatedAbility-skill integration (Conk/Slam/Berserk/Swipe), LongBlades 3-stance toggle, dual-wielding. Each is its own milestone.
