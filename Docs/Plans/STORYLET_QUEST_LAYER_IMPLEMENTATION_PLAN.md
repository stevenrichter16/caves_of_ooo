...
| 2026-04-26 | M2 | ✅ | StoryletPart on world entity (ISaveSerializable + INarrativeReactor stub), QuestState POCO, GameBootstrap wires on fresh boot AND on load (incl. RegisterReactor on NarrativeStatePart), FormatVersion bumped 2→3. 21 new EditMode tests green; 2282/2282 total. RED→GREEN confirmed (3 CS0246 compile errors observed before production code). |
| 2026-04-27 | M3 | ✅ | StoryletPart.OnTickEnd implements single-pass dispatch — snapshot eligibility at top of tick, fire effects after; cascading triggers land NEXT tick. OneShot fires once; non-OneShot fires every tick predicates pass; quest storylets skipped (M4 lands those). Reuses ConversationPredicates.CheckAll + ConversationActions.ExecuteAll. 16 new EditMode tests green (12 expected RED → GREEN, 4 counter-checks); 2298/2298 total. |
