...
| 2026-05-04 | Bug A fix | ✅ Shipped | Same M7-prep PlayMode sweep caught: scene rendered correctly but had black bars on the right + bottom. Reflection probe of the live editor (Camera.main pos (40, 12.5, -10), ortho 13, aspect 1.778) ruled out viewport-overshoot. Real root cause: `SidebarCamera` and `HotbarCamera` (both `clearFlags=SolidColor`, `backgroundColor=Color.black` per `CameraFollow.ConfigureSidebarCamera/Hotbar`) keep painting their full pixelRects while the gameplay camera renders the scene composition only inside its narrower MapRect — so the screen-space sidebar and hotbar areas look like opaque black bars around the scene. Fix uses the existing public `CameraFollow.SetUIView(W, H)` (expands gameplay camera to fullscreen, disables sidebar/hotbar/popup-overlay cameras, centers on scene canvas) on activation, and pairs with `RestoreGameView()` once the reverse dissolve completes (NOT on HandleDeactivated — would jump-cut the world's framing mid-iris). New inspector field `SceneViewUI.CameraFollow` mirrors the existing `ZoneRenderer` field's auto-discovery via FindObjectOfType in Awake. 2 new EditMode tests in `SceneViewUICameraFramingTests.cs` (the file is also a place for future SceneViewUI MonoBehaviour-lifecycle tests, addressing one of M4's 🧪 deferred items): positive (Activate "Campfire" → SidebarCamera + HotbarCamera both disabled) + counter-check (Activate unknown SceneID → both stay enabled). 75 / 75 SceneView + InputHandler tests green (the 2 new SceneViewUI tests are inside the 68-test SceneViews namespace count). 🔵 / 🧪 findings in commit body; no 🟡+. Resolves "Open design consideration #1" from the plan ("Camera framing for the Scene View"). Visual re-verification pending. |
| TBD | M7 | ⏳ Pending | — |
