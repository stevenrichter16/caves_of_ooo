...
| 2026-05-04 | M4 | ✅ Shipped | Dissolve transition: radial iris from center (`cxx=W/2`, `cyy=H/2+2`) over `DISSOLVE_DURATION=1.6f` seconds, soft edge of width 1 cell at the reveal boundary. SceneRenderer gains `StartDissolve(bool reverse)`, `UpdateDissolve(float deltaTime)`, `GetMask(x,y)`, plus read-only state `IsDissolving`/`DissolveIsReverse`/`DissolveProgress`. Mask drives a `DrawDissolveOverlay` composition pass that runs only when `IsDissolving` is true (preserves M2/M3 baseline behavior when no dissolve is active). SceneViewUI lifecycle: forward dissolve on activate (ZoneRenderer left unpaused so world tilemap peeks through cleared overlay cells, then paused once forward dissolve completes); reverse dissolve on deactivate (ZoneRenderer unpaused; render-cleanup deferred until reverse completes via `_exitDissolveActive` flag). 14 new EditMode tests cover initial state + StartDissolve flag, forward/reverse mask shapes at p=0/mid/p=1, per-cell temporal monotonicity (counter-check pair), post-completion no-op, render integration (no-dissolve baseline + full-mask cell-clear), OOB-safe `GetMask`. **Scope cuts from plan**: (a) `SceneViewManager.cs` Begin/End hooks not added — manager remains pure state machine, SceneViewUI manages its own lifecycle (same pattern as M1's TurnManager cut). (b) Pre-pattern function (`PreChar`/`PreColor` from JS) not implemented — overlay tilemap renders above the world tilemap (sortingOrder=100 vs 0), so cleared cells (`Glyph=' '`) expose the real world below for a true "world ↔ scene" transition; pre-pattern would have hidden the world. Both cuts documented in commit body per §6.4. All 56 SceneView tests pending verification (Unity MCP detached during M4 work). |
| TBD | M5 | ⏳ Pending | — |
| TBD | M6 | ⏳ Pending | — |
| TBD | M7 | ⏳ Pending | — |
