...
7. **SM7 — On-hit-dispatch-requires-survival timing fix (B1).** The highest-risk item in this plan: move `OnHitClassEffects`/`OnHitWeaponEffects`/`OnHitGasEmit`/`ItemEnhancementDispatch` to fire immediately once the to-hit roll connects — before the `penetrations == 0` and `damage.Amount <= 0` early-returns — matching Qud's `WeaponHit` timing (fires before `Penetrations` is finalized). Keep `AttackerAfterAttack`/`WeaponMadeCriticalHit` skill dispatch and `CheckCombatDismemberment` gated on `hpAfter > 0` survival, unchanged. **Explicit behavior change, not a bug fix in the narrow sense:** a Serrated dagger's bleed chance, an EmberSpear's Burning spec, and any gas-emit/item-enhancement proc can now roll on a fully-armor-blocked (0-penetration or 0-damage) hit, and on a killing blow — neither was possible before. Already implicitly authorized by the user naming "on-hit timing" Qud-parity drift as in-scope; documented here for transparency per the project's scope-divergence convention, not as a gate requiring further sign-off. Tests: (a) a heavily-armored target (guaranteed 0 penetrations) still rolls/can trigger a weapon-mod effect; (b) a killing blow still rolls/can trigger one; (c) counter-check — skill dispatch (`AttackerAfterAttack`) and dismemberment do NOT fire on a dead target, confirming those two stay survival-gated as designed.
