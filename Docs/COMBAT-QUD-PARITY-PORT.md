...
- ✅ **PlayMode sanity sweep for F/G/H** — first attempt blocked by Unity's editor-focus-deferred domain reload; after user restarted the editor, the sweep ran cleanly. 3 scenarios all pass: Phase F (10 dmg → 8 via StoneSkin listener), Phase G (`GetOffHandHitBonus` arithmetic verified across stat=±N), Phase H (Indestructible vetoes 30/30 trials, control 14/30 ≈ 46.7% saturated). See `COMBAT-PARITY-PORT-REVIEW.md` "Phase F/G/H methodology-debt closure" section for raw output. The editor-focus-deferral pitfall is now documented in `CLAUDE.md`.
