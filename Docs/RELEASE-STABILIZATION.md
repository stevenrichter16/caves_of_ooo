...
### R6 acceptance — 19 September 2026, the ending spine, local main promotion

Transparent undertaken/refused/abandoned semantics and the smallest complete route to an enacted ending: the closure-ledger (ES.1), visible with a spoken no at every giver and regional requests included (ES.2), lost givers recognised and renounceable with the reading saying where to end each open act (ES.3), the circle's two enactments at the Felling-Site's seventh position (ES.4), the adversarial sweep (ES.5) and native proof (ES.6). Six sub-milestones, each RED→GREEN in the independent clone and committed separately (ES.1 `e4c25f8b`, ES.2 `a5c8ef37`, ES.3 `0109fb0e`, ES.4 `0b5bf93c`, ES.5 `d6c5e105`, ES.6 this promotion; the ES.3–ES.5 doc sections were published late by the docs-only `a8fe394e`, see the doc's process finding); see [ENDING-SPINE](ENDING-SPINE.md) for every gate and self-review. **Full EditMode 15,159/15,159**, zero compiler errors; **native journey 113/113 checks, validated over 1,460 native steps** with 36 captures inspected, enacting the practice path with the ledger clean (4 closed, 0 refused, 0 open). Bounds: one enactment per run (the vessel path and the open-ledger refusal are EditMode-covered, not captured); faction-level consequences stated in the epilogue, not enacted by NPCs; no ambient sari hook; Consume and Preserve not built; the shipped village quest definitions carry no display names. No epilogue text resolves a Mystery Ledger entry.

### R7 acceptance — 19 September 2026, the other advertised routes, local main promotion

The two advertised ending routes that could not be reached now can: the Root as a place (ER.1), the three name-holding stones and Kept (ER.2), the Choir's cutting and Gathered (ER.3), both acts on the ledger and in the journal (ER.4), the adversarial sweep (ER.5) and native proof (ER.6). Six sub-milestones, each RED→GREEN in the independent clone and committed separately (ER.1 `312bd7b7`, ER.2 `6119f888`, ER.3 `90c0610b`, ER.4 `308df661`, ER.5 `e8a97358`, ER.6 this promotion); see [ENDING-ROUTES](ENDING-ROUTES.md) for every gate, finding and self-review. **Full EditMode 15,223 tests, 15,223 passed**, zero compiler errors; **two native journeys 119/119 and 119/119 checks, both validated, 1,465 and 1,472 native steps** with 38 + 40 captures inspected, one enacting Kept over a ledger holding the Keep act, one reading both offers and refusing the seal for want of stones before the practice-path Renewal. Bounds: Gathered offered natively, enacted in EditMode only; the deep Choir and Recension arcs (Selen's name, the reconciliation, Dohren's gate) not built; consequences stated, not enacted by NPCs; the disk filled once mid-run during ER.1 (its first full run lost its receipt and was rerun; recorded in the doc). No epilogue text resolves a Mystery Ledger entry.
