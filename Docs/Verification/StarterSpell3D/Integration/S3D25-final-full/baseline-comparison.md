...
**PASS: all 31 failing test names exactly match the pre-integration S3D00b baseline.**

Baseline: 10902/10933 passing. S3D25: 11161/11192 passing. Both have 31 failures and zero skipped cases. Both receipts record zero compiler errors.

There are **259 added test cases; all pass**. New failures: **0**. Removed baseline cases: **0**. No failure was ignored or matched by a shortened name.

The exact names and old/new failure messages are preserved in `baseline-comparison.json`. This closes the source state tested by S3D25; the pending ground-marker refinement requires its own later verification. S3D24 remains a source-specific native capture.
