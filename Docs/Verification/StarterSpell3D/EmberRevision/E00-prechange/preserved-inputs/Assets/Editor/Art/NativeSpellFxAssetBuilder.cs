...
namespace CavesOfOoo.Editor
{
    ...
    public static class NativeSpellFxAssetBuilder
    {
        ...
        [Serializable] sealed class MaterialData { public string id; public float[] rgbaSrgb; public float emission; public bool glow; }
        [Serializable] sealed class MeshData { public string id; public float[] vertices, normals, vertexColors; public int[] triangles; public int materialIndex; }
        ...
        [Serializable] sealed class Piece { public string id, meshId, role, anchor, condition, visualBounds; public int forwardCell, lateralCell, variant; public bool reducedEssential; public Pose[] poses; }
        ...
        public static void Import(string reportPath)
        {
            ...
            try
            {
                ...
                var luminous = SpellMaterial(Art + "/Materials/SpellLuminous.mat", "CavesOfOoo/Spell3D/Luminous");
                var glow = SpellMaterial(Art + "/Materials/SpellGlow.mat", "CavesOfOoo/Spell3D/SoftGlow");
                var meshes = new Dictionary<string, Mesh>(StringComparer.Ordinal);
                var meshRows = data.meshes.ToDictionary(m => m.id, StringComparer.Ordinal);
                ...
                foreach (var row in data.meshes)
                {
                    ...
                    mesh.vertices = Triples(row.vertices); mesh.normals = Triples(row.normals); mesh.triangles = row.triangles;
                    if (row.vertexColors != null && row.vertexColors.Length > 0)
                    {
                        var gradient = new Color[row.vertexColors.Length / 4];
                        for (int i = 0; i < gradient.Length; i++) gradient[i] = new Color(row.vertexColors[i*4], row.vertexColors[i*4+1], row.vertexColors[i*4+2], row.vertexColors[i*4+3]);
                        mesh.colors = gradient;
                    }
                    mesh.RecalculateBounds(); Save(mesh);
                ...
                foreach (var study in data.studies)
                {
                    ...
                    foreach (var p in study.pieces)
                        pieces.Add(new NativeSpellFxPiece {
                            ...
                            Emission = data.materials[meshRows[p.meshId].materialIndex].emission,
                            Glow = data.materials[meshRows[p.meshId].materialIndex].glow,
                            VisualBounds = string.IsNullOrEmpty(p.visualBounds) ? NativeSpellFxBounds.CellSurface : (NativeSpellFxBounds)Enum.Parse(typeof(NativeSpellFxBounds), p.visualBounds),
                ...
                library.Material = material; library.LuminousMaterial = luminous; library.GlowMaterial = glow; library.Entries = entries.ToArray(); library.Validate(); Save(library);
                report.status = "PASS";
            }
        ...
        static Material SpellMaterial(string path, string shaderName)
        {
            var shader = Shader.Find(shaderName); Require(shader != null, "Missing spell shader " + shaderName);
            var material = Owned(path, () => new Material(shader)); material.shader = shader;
            material.SetTexture("_BaseMap", Texture2D.whiteTexture); material.SetTexture("_FogLight", Texture2D.blackTexture);
            material.SetColor("_BaseColor", Color.white); material.SetFloat("_Transient",1); material.SetFloat("_Emission",0);
            material.enableInstancing = true; Save(material); return material;
        }
        // Validate new color/material metadata before creating or replacing owned
        // assets. Silent loss of Blender emission or alpha is an import failure.
        static void ValidateExport(Export data)
        {
            Require(data.materials != null && data.materials.Length > 0 && data.meshes != null, "Missing material/mesh export.");
            foreach (var material in data.materials)
            {
                Require(material != null && material.rgbaSrgb != null && material.rgbaSrgb.Length == 4
                    && material.rgbaSrgb.All(Unit) && material.rgbaSrgb[3] == 1, "Invalid authored material color.");
                Require(Unit(material.emission) && (!material.glow || material.emission > 0), "Invalid authored emission.");
            }
            var meshes = new Dictionary<string, MeshData>(StringComparer.Ordinal);
            foreach (var mesh in data.meshes)
            {
                Require(mesh != null && !string.IsNullOrWhiteSpace(mesh.id) && !meshes.ContainsKey(mesh.id), "Missing/duplicate exported mesh.");
                SafeName(mesh.id);
                Require(mesh.vertices != null && mesh.vertices.Length >= 9 && mesh.vertices.Length % 3 == 0
                    ...
                    && mesh.vertices.All(Finite) && mesh.normals.All(Finite), "Malformed exported geometry.");
                Require(mesh.triangles != null && mesh.triangles.Length >= 3 && mesh.triangles.Length % 3 == 0
                    && mesh.triangles.All(i => i >= 0 && i < mesh.vertices.Length/3), "Malformed exported triangle indices.");
                Require(mesh.materialIndex >= 0 && mesh.materialIndex < data.materials.Length, "Invalid exported material index.");
                var colors = mesh.vertexColors;
                bool hasColors = colors != null && colors.Length > 0;
                Require(!hasColors || (colors.Length == mesh.vertices.Length/3*4 && colors.All(Unit)), "Malformed exported vertex color gradient.");
                if (data.materials[mesh.materialIndex].glow)
                {
                    Require(hasColors, "Glow mesh has no authored gradient.");
                    float min=1,max=0;
                    for(int i=3;i<colors.Length;i+=4) { min=Math.Min(min,colors[i]);max=Math.Max(max,colors[i]); }
                    Require(min<=.001f&&max>.001f,"Glow gradient lacks transparent edges or visible core.");
                }
                meshes.Add(mesh.id,mesh);
            }
            foreach (var study in data.studies)
            {
                Require(study != null && study.pieces != null && study.pieces.Length > 0, "Missing exported study pieces.");
                var ids = new HashSet<string>(StringComparer.Ordinal);
                foreach (var piece in study.pieces)
                {
                    Require(piece != null && !string.IsNullOrWhiteSpace(piece.id) && ids.Add(piece.id)
                        && piece.meshId != null && meshes.ContainsKey(piece.meshId), "Missing/duplicate exported piece.");
                    Require(Enum.TryParse(piece.role, out NativeSpellFxRole role) && Enum.IsDefined(typeof(NativeSpellFxRole),role)
                        && Enum.TryParse(piece.anchor,out NativeSpellFxAnchor anchor) && Enum.IsDefined(typeof(NativeSpellFxAnchor),anchor)
                        && Enum.TryParse(piece.condition,out NativeSpellFxCondition condition) && Enum.IsDefined(typeof(NativeSpellFxCondition),condition), "Unknown exported piece semantics.");
                    Require(string.IsNullOrEmpty(piece.visualBounds) || (Enum.TryParse(piece.visualBounds,out NativeSpellFxBounds bounds)
                        && Enum.IsDefined(typeof(NativeSpellFxBounds),bounds)), "Unknown exported visual bounds.");
                    Require(piece.poses != null && piece.poses.Length == NativeSpellFxLibrary.SampleCount, "Invalid exported pose count.");
                    foreach(var pose in piece.poses)
                    {
                        Require(pose != null && pose.position != null && pose.position.Length == 3 && pose.position.All(Finite)
                            && pose.scale != null && pose.scale.Length == 3 && pose.scale.All(v=>Finite(v)&&v>=0)
                            && pose.rotation != null && pose.rotation.Length == 4 && pose.rotation.All(Finite)
                            && Math.Abs(pose.rotation.Sum(v=>v*v)-1)<.01f, "Invalid exported sampled transform.");
                    }
                }
            }
        }
