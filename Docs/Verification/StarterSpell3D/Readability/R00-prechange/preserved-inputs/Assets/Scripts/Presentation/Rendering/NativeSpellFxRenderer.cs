...
namespace CavesOfOoo.Rendering
{
    ...
    public sealed class NativeSpellFxRenderer : IDisposable
    {
        ...
        private sealed class View { public Transform Transform; public MeshFilter Filter; public MeshRenderer Renderer; }
        ...
        public bool IsPrepared => !_disposed && SpellFxSettings.Mode != SpellFxMode.Off && SurfaceReady
            && _zone != null && _library != null && _library.IsValidated && _root != null && _material != null && _views.Count >= Budget;
        ...
        public void Update(float playbackDelta)
        {
            ...
            for (int c = 0; c < _casts.Count && drawn < Budget; c++)
            {
                ...
                for (int i = 0; i < cast.Fragments.Count && drawn < Budget; i++)
                {
                    ...
                    _properties.SetVector(BaseColor, (Vector4)color); view.Renderer.SetPropertyBlock(_properties);
            ...
            ref Vector3 position, ref Quaternion rotation, ref Vector3 scale)
        {
            ...
            else
            {
                ...
                if (perCell && cast.Dx != 0 && cast.Dy != 0)
                {
        ...
        private void EnsureRoot()
        {
            ...
            if (_material == null)
            {
                _material = new Material(_library.Material) { name = "Native spell transient (owned)", hideFlags = HideFlags.DontSave };
                _material.SetTexture("_BaseMap", Texture2D.whiteTexture);
                _material.SetTexture("_FogLight", _surface.FogTexture);
                _material.SetFloat("_Transient", 1f);
            }
        ...
        private void CreateView()
        {
            ...
            _views.Add(new View { Transform = go.transform, Filter = go.GetComponent<MeshFilter>(), Renderer = renderer });
        }
