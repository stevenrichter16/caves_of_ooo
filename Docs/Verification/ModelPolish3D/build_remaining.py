...
base=['/Applications/Blender.app/Contents/MacOS/Blender','-b','--threads','2','--python-exit-code','1','--python']
for kit,label in [('Village3D','village'),('SpawnRing3D','ring'),('MultiCellPilot3D','pilot')]:
 dest=Path('/tmp/coo-model-polish-final')/kit
 if kit!='Village3D':
  cmd=base+[str(root/'ArtSource/ModelPolish3D/polish_bundle.py'),'--','--kit',kit,'--source','/tmp/coo-model-polish-baselines/'+kit,'--output',str(dest),'--render']
  with (out/('final-'+label+'-build.log')).open('w') as f:subprocess.run(cmd,stdout=f,stderr=subprocess.STDOUT,check=True)
 with (out/('final-'+label+'-roundtrip-final.log')).open('w') as f:subprocess.run(base+[str(root/'ArtSource/ModelPolish3D/validate_bundle.py'),'--',str(dest)],stdout=f,stderr=subprocess.STDOUT,check=True)
