    ...
    public sealed class WorldFxCoordinator : IDisposable
    {
        ...
        public WorldFxPlayback Play(SpellFxSequence sequence)
        {
            ...
            try
            {
                ...
                if (duration <= 0f && _spriteMode) duration = _sprites.Play(sequence, definition);
                if (duration <= 0f) duration = PlayAscii(sequence, definition);
        ...
        private float PlayAscii(SpellFxSequence sequence, SpellFxDefinition definition)
        {
