# Village regression runners — source-reviewed handoff

These wrappers are prepared for the final Village checks after ring/shared-surface integration. They have **not launched Unity or restarted MCP**. Default invocation prints a plan and has no process or filesystem side effects. Only `--execute` runs an editor.

Run sequentially after other editors/batches have exited and all source/asset edits are frozen:

```sh
python3 /tmp/codex-village-regression-runners/run_gpu.py --execute
python3 /tmp/codex-village-regression-runners/run_native.py --execute
```
