...
| WSP7.3 | (this commit) | **Elemental retort powers — first defender-side magic.** 4 new powers (one per damage-elemental tree) that fire from `OnDefenderAfterAttackMissed` when an attack on the defender misses. Each deals a small flat (RETORT_DAMAGE = 3) elemental damage to the original attacker, tagged with the corresponding element so the resistance pipeline fires correctly: `Pyromancy_ScorchRetort` (Heat), `Cryomancy_FrostRetort` (Cold), `Galvanism_ShockRetort` (Electric), `Corrosion_AcidRetort` (Acid). All 4 use the existing OnDefenderAfterAttackMissed hook (parallel to `ShortBlades_Rejoinder`'s defender-side counter-attack pattern, but with elemental damage instead of a weapon swing — no recursion-guard needed because the retort goes through `CombatSystem.ApplyDamage` directly, not back through `PerformSingleAttack`). 11 RED→GREEN tests including resistance-tagging verification (100% HeatResistance attacker takes 0 damage from ScorchRetort) + 4-skill stacking pin (defender with all 4 retorts deals 4×3=12 cumulative damage via `SkillEventDispatcher.DefenderAfterAttackMissed`) + JSON load. | +4 |
