...
| WSP7.0 | (this commit) | **Begin the magic & grimoire skill suite.** Adds new `OnGetSpellDamageModifier` virtual to BaseSkillPart + `GetSpellDamageModifier` dispatcher entry-point — the second new combat-event hook since the system shipped (after WSP6.6's pen-modifier hook), again following the §"Adding a new combat event" mechanical pattern. Adds new `MutationDamageHelpers.ApplySpellDamage` shared damage path that all migrated mutations use. Migrated 6 directional projectile mutations (FireBolt → Fire, IceLance → Cold, IceShard → Cold, ArcBolt → Electric, AcidSpray → Acid, Kindle → Fire) — pre-WSP7 these mutations passed raw int damage to ApplyDamage's int-overload, which built a Damage with NO attributes, silently bypassing the Heat/Cold/Electric/Acid resistance pipeline. **WSP7.0 fixes that real bug** as a side benefit of the magic-skill infrastructure. Ships 2 magic skill trees: `SpellcraftSkill` (universal +1 spell damage, regardless of element — CoO-Original Extension; Qud has no parallel) and `PyromancySkill` (+25% Heat damage to Burning targets — CoO-Original Extension). 15 RED→GREEN tests including resistance-bug-fix verification + Spellcraft+Pyromancy stacking pin + JSON content load. Trees use the `<Tree>Skill` convention (parallel to AcrobaticsSkill / CudgelSkill / etc.). | +2 |
