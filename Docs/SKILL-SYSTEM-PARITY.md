...
| WSP7.2 | (this commit) | **Magic-melee crossover powers — first hybrid build support.** 4 new powers (one per damage-elemental tree) that grant +50% bonus melee damage when the defender has the corresponding elemental status effect, applied as a separate elemental-tagged damage call: `Cryomancy_BrittleStrike` (Frozen → bonus Cold damage), `Galvanism_GroundStrike` (Electrified → bonus Electric), `Pyromancy_Charsplit` (Charred → bonus Heat), `Corrosion_Etch` (Acidic → bonus Acid). All four use the existing `OnAttackerAfterAttack` melee hook + the existing on-hit-proc passive shape (mirrors `Cudgel_Bludgeon`'s structure). No new infrastructure. Rewards "spell setup → melee follow-up" play patterns and synergizes with the WSP7.1 elemental tree-roots (e.g., Cryomancy buffs the IceLance that Freezes; BrittleStrike buffs the melee that follows). 13 RED→GREEN tests including a full-stack pin (4 powers on 1 hyper-effected target = 4×5=20 bonus HP via the real `SkillEventDispatcher.AttackerAfterAttack` path) + cross-skill non-interference + JSON load. | +4 |
