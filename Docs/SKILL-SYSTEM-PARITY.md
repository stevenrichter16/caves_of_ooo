...
| WSP7.1 | `f5d52f4` (merge) | **Magic suite expansion — 3 more elemental tree-roots + 2 power skills.** Ships: `CryomancySkill` (+25% Cold damage to Wet/Frozen, accepts Cold/Ice/Freeze aliases), `GalvanismSkill` (+25% Electric damage to Wet/Electrified, accepts Electric/Lightning/Shock aliases), `CorrosionSkill` (+25% Acid damage to Acidic-stacked), `Pyromancy_Cinder` (+33% Heat damage to Charred — post-Burning state rewarder), `Spellcraft_Empower` (universal +2 stacks with Spellcraft root for +3 total). 20 tests. | +5 |
| WSP7.2 | `6cd18cc` (merge) | **Magic-melee crossover powers — first hybrid build support.** 4 new powers (one per damage-elemental tree) that grant +50% bonus melee damage when the defender has the corresponding elemental status effect: `Cryomancy_BrittleStrike` (Frozen), `Galvanism_GroundStrike` (Electrified), `Pyromancy_Charsplit` (Charred), `Corrosion_Etch` (Acidic). 13 tests. | +4 |
| WSP7.3 | `4198d3b` (merge) | **Elemental retort powers — first defender-side magic.** 4 new powers (`Pyromancy_ScorchRetort`, `Cryomancy_FrostRetort`, `Galvanism_ShockRetort`, `Corrosion_AcidRetort`) that fire from `OnDefenderAfterAttackMissed` and deal small flat elemental damage to the attacker. 11 tests. | +4 |
| WSP7.4 | (this commit) | **Spell resistance audit + migration.** Pre-WSP7.4 audit revealed 9 elemental damage call sites that silently bypassed the resistance pipeline (used the int-overload of `CombatSystem.ApplyDamage` which built a Damage with no attributes). This commit ships the fix: (1) Adds new canonical overload `CombatSystem.ApplyDamage(target, amount, elementAttribute, source, zone)` that wraps in a typed Damage with the element attribute set. (2) Migrates 8 AOE mutations to use it: `ChainLightning` → Electric, `Thunderclap` → Electric, `FlamingHands` → Heat, `PrismaticBeam` → Light, `EmberVein` → Heat, `FrostNova` → Cold, `Conflagration` → Heat, `RimeNova` → Cold. (3) Migrates `MaterialReactionResolver`'s "DealDamage" effect type to read element from `ReactionEffect.StringValue` (backward-compatible repurpose — empty string preserves the old untyped behavior). (4) Updates 7 reaction JSON files to specify the right element on their DealDamage entries (cold_plus_metal/crystal/chitinous → Cold, acid_plus_organic → Acid, fire_plus_ice/fungal/bone → Heat). (5) Audited remaining sites: BurningEffect already correctly tagged "Fire", AcidicEffect already correctly tagged "Acid", ElectrifiedEffect already correctly tagged "Lightning"; BleedingEffect / PoisonedEffect / MaterialPart shatter intentionally remain untagged (no matching resistance system). 14 RED→GREEN tests covering per-element pin + element-alias acceptance ("Fire" → Heat flag, "Ice" → Cold, "Lightning" → Electric) + cross-element no-interference + null/empty-element bypass + ElectrifiedEffect/BurningEffect/AcidicEffect tick verification. | n/a |
