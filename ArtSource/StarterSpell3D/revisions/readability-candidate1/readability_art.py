...
def author(a,spec,root,aftermath,target,cast,contact,clear):
 ...
 if sid in ('ember_spit','flaming_hands'):
  ...
  for j in range(5):
   radius=.98+j*.10;azimuth=math.radians(-65+j*32.5);ps=[]
   for k in range(17):
   ...
   paths.append(ps);r=[(.07+.025*j)*math.sin(math.pi*(k+.8)/18)**.7 for k in range(17)]
  ...
  def flow(u,f):
   width=.24+.59*(u-.40);y=f*width
   curl=max(0,(u-2.22)/.52);x=-2.35+u-.16*curl*curl
   z=.46+.10*math.sin(u*2.3)+.075*math.cos(f*8+u*3)+.53*curl*curl
   return Vector((x,y,z))
  for ci,(forward,lateral) in enumerate([(1,0),(2,-1),(2,0),(2,1)]):
   ...
   if forward==2:
    ...
    tube('RD_Jet_cell_%d_%d_front_curl'%(forward,lateral),lip,.083,'river_pearl',**opts)
   halo('RD_Jet_cell_%d_%d_mist'%(forward,lateral),paths[1],.29,'river_glow',**opts)
   cluster_paths('RD_Jet_cell_%d_%d'%(forward,lateral),paths,['river_pearl','river_mint','river_turquoise'],cohorts=3,count=12,spread=.12,scale=.060,**opts)
  ...
  for j in range(7):
   angle=.10+j*math.tau/7;ps=[]
   for k in range(7):
    t=k/6;r=.63+t*.82;x=math.cos(angle)*r;y=math.sin(angle)*r;z=.18+t*1.45+.18*math.sin(t*math.pi)
    ps.append(tuple(target+Vector((x,y,max(.17,z)))))
   paths.append(ps);r=[.10,.16,.18,.16,.12,.085,.025]
  ...
  for j,(begin,length,radius) in enumerate([(-2.85,1.70,1.32),(-.95,1.70,1.32),(1.0,1.50,1.32)]):
   ps=[]
   for k in range(18):
    t=k/17;angle=begin+t*length;ps.append((math.cos(angle)*radius,math.sin(angle)*radius,.48*math.sin(angle*2)+.22*math.cos(angle)))
