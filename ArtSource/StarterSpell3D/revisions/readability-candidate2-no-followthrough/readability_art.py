...
def author(a,spec,root,aftermath,target,cast,contact,clear):
 ...
 if sid in ('ember_spit','flaming_hands'):
  ...
  # Thin swept tongues spread through several azimuths. Broad glow/motes carry
  # volume; no three solid horizontal prongs or rigid circular pipe silhouette.
  for j in range(7):
   azimuth=math.radians(-74+j*24.7);ps=[];height=1.55+.27*math.sin(j*1.4)
   for k in range(20):
    t=k/19;x=-1.12+2.32*t-.58*t*t;z=.16+height*math.sin(t*math.pi*.60);x+=.20*math.sin(t*math.pi*1.8+j*.45);y=.14+.12*math.sin(t*math.pi)
    x*=1.90;z*=1.16;ps.append(tuple(target+Vector((x*math.cos(azimuth)-y*math.sin(azimuth),x*math.sin(azimuth)+y*math.cos(azimuth),z))))
   paths.append(ps);r=[.013+(.10+.024*(j%3))*math.sin(math.pi*(k+.15)/20)**1.05 for k in range(20)]
   tube('RD_Fire_%s_swept_tongue_%d'%(sid,j),ps,r,'molten_gold' if j%3!=1 else 'fire_coral',flatten=.33,**opts)
   if j in (0,2,4,6):
    inner=[tuple(Vector(p)+Vector((-.017,-.025,.018))) for p in ps];tube('RD_Fire_%s_hot_ridge_%d'%(sid,j),inner,[q*.30 for q in r],'hot_ivory',flatten=.6,**opts)
  glowbed('RD_Fire_%s_soft_bed'%sid,target,2.48,'fire_glow',**opts)
  halo('RD_Fire_%s_crown_halo'%sid,paths[3],.55,'fire_glow',**opts)
  cluster_paths('RD_Fire_'+sid,paths,['hot_ivory','molten_gold','fire_coral'],cohorts=12 if hands else 10,count=12,spread=.22,scale=.048,**opts)
  if not hands:
   ...
   cluster_paths('RD_Ember_stream',[trail],['hot_ivory','molten_gold'],cohorts=4,count=12,spread=.12,parent=carrier,scale=.038,role='ProjectileHead',anchor='ProjectileCarrier',start=14,hold=contact+10)
 elif sid=='jet_blast':
  ...
  for ci,(forward,lateral) in enumerate([(1,0),(2,-1),(2,0),(2,1)]):
   ...
   cluster_paths('RD_Jet_cell_%d_%d'%(forward,lateral),paths,['river_pearl','river_mint','river_turquoise'],cohorts=4,count=12,spread=.16,scale=.045,**opts)
  ...
  cluster_paths('RD_Rime',paths,['frost_white','frost_blue','frost_white'],cohorts=15,count=12,spread=.19,shape='ice',scale=.049)
 elif sid=='calm':
  ...
  cluster_paths('RD_Calm',outpaths,['calm_ivory','calm_lilac','calm_violet'],cohorts=15,count=12,spread=.17,shape='leaf',scale=.046,condition='PacifiedApplied')
