...
def matindex(ob):
 ...
 index=len(materials);materials.append(dict(id=mat.name,rgbaSrgb=[round(v,7) for v in srgb]+[1.0],emission=float(mat.get('emission',0)),glow=bool(mat.get('glow',False))));matids[mat.name]=index;return index
...
def triangle_colors(ob):
 me=ob.data;me.calc_loop_triangles();attr=me.color_attributes.get('SpellGlow')
 if not attr:return None
 if attr.domain!='POINT':raise ValueError('SpellGlow must use POINT colors: '+ob.name)
 return [[list(attr.data[index].color) for index in tri.vertices] for tri in me.loop_triangles]
def addmesh(name,tris,material,colors=None):
 vertices=[];normals=[];indices=[];rgba=[]
 for ti,tri in enumerate(tris):
  ...
  if colors:rgba.extend(round(float(v),7) for color in colors[ti] for v in color)
 if not indices:return None
 index=len(meshes);meshes.append(dict(id=name,vertices=vertices,normals=normals,triangles=indices,materialIndex=material,vertexColors=rgba));return name
...
def trs_tracks(scene,ob,anchor):
 ...
 return result
def piece(name,mesh,role,condition,anchor,poses,forward=0,lateral=0,variant=0,essential=True,adaptation='',bounds='CellSurface'):
 return dict(id=name,meshId=mesh,role=role,condition=condition,anchor=anchor,forwardCell=forward,lateralCell=lateral,variant=variant,reducedEssential=essential,nativeAdaptation=adaptation,visualBounds=bounds,poses=poses)
...
for spec in manifest['studies']:
 ...
 for ob in effects:
  ...
  if ob.get('nativeRole'):
   role=ob['nativeRole'];condition=ob['nativeCondition'];anchorkey=ob['nativeAnchor'];forward=int(ob['nativeForwardCell']);lateral=int(ob['nativeLateralCell']);variant=int(ob['nativeVariant'])
   anchor=SOURCE if anchorkey in ('Source','ProjectileCarrier') else target if anchorkey=='Target' else SOURCE+Vector((forward,lateral,0))
   native_mesh=addmesh(name,triangles(ob),mat,triangle_colors(ob));poses=trs_tracks(scene,ob,anchor)
   pieces.append(piece(name,native_mesh,role,condition,anchorkey,poses,forward,lateral,variant,bool(ob['reducedEssential']),bounds=ob['visualBounds']))
   continue
  ...
  mesh=addmesh(name,triangles(ob),mat,triangle_colors(ob));poses=trs_tracks(scene,ob,anchor)
...
for entry in library['studies']:
 ...
 if entry['id'] in ('flaming_hands','jet_blast'):
  ...
  for p in entry['pieces']:
   if p['visualBounds']=='AirborneDecoration':continue
