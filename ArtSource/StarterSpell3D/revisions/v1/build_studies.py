...
def pulse(ob,start,contact,finish,scale=None):
 ...
 key(ob,0,scale=0);key(ob,max(0,start-1),scale=0);key(ob,start+2,scale=base);key(ob,contact,scale=base);key(ob,finish,scale=0);key(ob,END,scale=0)
 EFFECTS.append(ob);ob['transientEffect']=True;ob['noCollider']=True;ob['clearFrame']=finish
def stage(spec):
...
def caster(spec):
 ...
 for frame,pose in [(0,{}),(12,gather),(22,release),(42,release),(66,{}),(END,{})]:
 ...
 if sid=='ember_spit':
  ...
  for side in [-1,1]:
   ...
   for j in range(4):
    ...
    key(ob,16,scale=(.03,.03,.03));key(ob,22,scale=1);key(ob,34,scale=1)
  ...
  for j in range(5):
   ...
   key(ob,22+j*1.2,scale=.08);key(ob,contact,scale=1)
  ...
  ob=path('Calm__violet_return',[(p[0]*1.17,.035,p[2]*1.13) for p in pts[3:14]],.027,'violet',loop);pulse(ob,16,contact+9,finish+4)
  ...
  for index,(x,y) in enumerate(beds):
   ...
   for f,a in [(0,0),(22,0),(28,.08),(33,0),(39,.06),(45,0),(END,0)]:key(leafroot,f,rotation=(a,0,0))
   for j in range(9):
    ...
    start=22+(j%3)*3; pulse(ob,start,start+12,finish+8)
    for f,z in [(start,.95),(start+9,.27),(start+10,.95),(start+19,.27)]:key(ob,f,location=(px,py,z))
   for j in range(3):
    a=j*2.1;pts=[(x+.16*math.cos(t),y+.16*math.sin(t),.22) for t in [a+k*.20 for k in range(7)]]
    ob=path('Rain__crop_%d_bead_arc_%d'%(index,j),pts,.012,'water_light',aftermath);pulse(ob,29,37,clear)
