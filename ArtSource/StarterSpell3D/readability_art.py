...
def author(a,spec,root,aftermath,target,cast,contact,clear):
 ...
 def own(ob,role='TargetImpact',condition='Always',anchor='Target',cell=(0,0),essential=True,bounds='AirborneDecoration',start=None,hold=None,drift=None):
  ...
  if drift:
   ...
   for frame,mix in [(contact-5,0),(contact+3,0),(contact+18,1),(clear,.38)]:
    if sid=='jet_blast':
     # Every patch rolls about one source pivot. Independent centroid/sign
     # rotations otherwise pry the shared lap seams apart during the hold.
     shared_turn=Vector((0,-.08,0))*mix;pivot=Vector((-2.35,0,0));delta=Euler(shared_turn).to_matrix()
     key(ob,frame,location=pivot+delta@(base-pivot)+Vector((push,0,rise))*mix,rotation=Vector(rot)+shared_turn)
    else:key(ob,frame,location=base+Vector((push,0,rise))*mix,rotation=Vector(rot)+turn*mix)
 ...
 if sid=='ember_spit':
  import ember_projectile_art
  ember_projectile_art.author(a,root,aftermath,target,cast,contact,clear)
  ...
  for ci,(forward,lateral) in enumerate([(1,0),(2,-1),(2,0),(2,1)]):
   opts=dict(role='ConeCell',anchor='Cell',cell=(forward,lateral));lo,hi=(.40,1.76) if forward==1 else (1.10,2.74)
   fa,fb=(-1,1) if forward==1 else (-1,-.08) if lateral<0 else (.08,1) if lateral>0 else (-.56,.56)
   ...
   if forward==2:
    ...
    # Internal ends feather into neighbouring laps; only the actual
    # outside rim retains its full width, avoiding outlined panel borders.
    weights=[min(1, .24+k*.22 if lateral>=0 else 1, .24+(15-k)*.22 if lateral<=0 else 1) for k in range(16)]
    tube('RD_Jet_cell_%d_%d_front_curl'%(forward,lateral),lip,[.115*w for w in weights],'river_pearl',**opts)
    rear=[tuple(Vector(p)+Vector((-.16,0,-.13))) for p in lip];tube('RD_Jet_cell_%d_%d_rolling_curl'%(forward,lateral),rear,[.18*w for w in weights],'river_mint',flatten=.55,**opts)
  ...
  for ci in range(4):
   ...
   # A current owner can reach backward toward its existing predecessor.
   # The last owner ends before its current root, avoiding false forward range.
   bridge=addvec([(-1.24,0,.24),(-.98,-.065,.26),(-.74,.06,.25),(-.47,-.07,.29),(-.20,.04,.24),(.06,-.025,.26),(.26,0,.24)],origin)
   tube('RD_Surge_cell_%d_air_stitch'%(ci+1),bridge,[.028,.055,.062,.07,.07,.06,.025],'root_ivory',start=contact-12,**opts)
   paths[0]=bridge;paths[1]=addvec(bridge,(0,.055,.035))
