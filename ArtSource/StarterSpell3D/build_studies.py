...
import readability_art
SOURCE=ROOT.parent/'Village3D/village_master.blend'
argslist=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else []
ap=argparse.ArgumentParser();ap.add_argument('--render-stills',action='store_true');ap.add_argument('--render-loops',action='store_true');ap.add_argument('--skip-export',action='store_true');ap.add_argument('--export-only',choices=['ember_spit']);args=ap.parse_args(argslist)
...
SPECS=[
 ...
 dict(id='flaming_hands',name='Flaming Hands',school='PYROMANCY',tag='OPEN / PRESS',distance=1,travel=False,color='flame',note='One adjacent ground contact; broad decorative upper flame fingers.'),
...
readability_art.install_materials(globals())
...
def effect_study(spec):
 ...
 if sid=='ember_spit':
  seed=empty('Ember__travel_seed',root)
 ...
 readability_art.author(globals(),spec,root,aftermath,target,cast,contact,clear)
...
if not args.skip_export:
 for study in STUDIES:
  if not args.export_only or study['spec']['id']==args.export_only:export(study)
...
manifest=dict(schemaVersion=1,purpose='Editable Blender source for native spell art; runtime uses sampled mesh/pose export',source=str(SOURCE.relative_to(ROOT.parents[1])),sourceSha256=SOURCE_HASH,cameraPitchDegrees=56,cameraSensorFit='HORIZONTAL',cameraPixelAspect=[1/math.sin(PITCH),1],outputPresentation='Display rendered PNG and composed videos at square pixels; the projection already compensates ground foreshortening.',unitsPerCell=1,blenderCoordinates='Z up; X is cast direction; FBX -Z forward / Y up',exportReadiness='FBX files are Blender-roundtrip design assemblies with retained studio offsets; runtime/starter_spell_library.json is the semantic native mesh/pose handoff. Native importer and GPU verification are performed separately.',nativeFps=FPS,previewPlayback='sample every third native frame, play at 8 fps (4.167x slower than native)',timingSeconds=dict(cast=.12,charge=.10,travelPerCell=.025,impact=.20,aftermath=.18),rootMotion=False,opaqueOnly=False,solidSurfacesOpaque=True,glowMaterials="Static emission/glow roles and white-RGB SpellGlow vertex alpha; no shader animation",readabilityRevision=1,noColliders=True,unrenderedOutcomeBranches=['miss / immune / denied status','lethal contact and break-away','blocked push vs successful displacement','wet fire suppression','Rime water-only fallback and dry refusal','Calm already peaceful / failed pacification','Rain no valid crops'],studies=[])
