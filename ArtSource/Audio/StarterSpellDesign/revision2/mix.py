...
def make(sid,v):
 ...
 if sid=='flaming_hands':
  ...
  l=s.layer('ground_fracture','A single stone-contact Foley accent gives ground weight; it does not report terrain destruction.')
  ...
  l=s.layer('inward_ice_stress','A low wooden-creak Foley proxy supplies pressure; reversed ice-pack fracture draws inward.')
  s.event(l,I+'wood-creak-ice-pressure-PROXY_SilentStrikeZ_public-hq-preview.mp3',.55+d*.065,.26,.012,rate=.63*detune,level=.075,lo=80,hi=1450,curve=[(0,0),(.055,.7),(.18,1),(.41,0)],attack=.025,release=.065)
  s.event(l,I+'ice-shatters/IceShatters/LedasLuzta4.ogg',.12,.24,.07,rate=1.05*detune,reverse=True,level=.072,lo=800,hi=5900,attack=.025,release=.018)
  ...
  for at,start,level in [(.25,.45,.020),(.43,2.06,.028),(1.12,3.66,.026),(1.54,5.12,.018),(1.82,.45,.013)]:s.event(l,W+'angeloyazar_water_drops.ogg',start,.19,at+d*.007,rate=1.25*detune,level=level,lo=420,hi=5500,attack=.004,release=.045)
 return s.finalize()
