...
class Study:
 ...
 def event(self,l,source,start,length,at,rate=1,level=.10,lo=60,hi=7500,reverse=False,curve=None,attack=.006,release=.03):
  ...
  l['events'].append({'source':source,'sourceSha256':hashlib.sha256((ROOT/source).read_bytes()).hexdigest(),'sourceStartSeconds':start,'sourceLengthSeconds':length,'placementSeconds':at,'rate':rate,'pitchSemitones':12*math.log2(rate),'reverse':reverse,'bandHz':[lo,hi],'referenceRms':level,'sourceNormalizationGain':gain,'attackSeconds':attack,'releaseSeconds':release,'curve':curve})
 def finalize(self):
...
def make(sid,v):
 ...
 if sid=='flaming_hands':
  ...
  s.event(l,I+'rubbed-singing-bowl_ryancacophony_public-hq-preview.mp3',7.2+d*1.3,2.8,.018,rate=.62*detune,level=.105,lo=85,hi=1900,curve=[(0,0),(.16,.55),(.34,.9),(.7,1),(1.45,.6),(2.32,0)],attack=.08,release=.12)
  l=s.layer('resolved_overtone','The same material opens into a quieter related resonance at actual pacification.')
  s.event(l,I+'rubbed-singing-bowl_ryancacophony_public-hq-preview.mp3',27+d*.7,2.4,.32,rate=.93*detune,level=.057,lo=220,hi=2500,curve=[(0,0),(.18,.65),(.54,1),(1.18,.6),(2.03,0)],attack=.10,release=.16)
