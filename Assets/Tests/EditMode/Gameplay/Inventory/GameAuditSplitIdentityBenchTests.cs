...
{
    public class GameAuditSplitIdentityBenchTests
    {
        private EntityFactory _oldFactory, _oldStill;
        private static IScenario Bench()
        {
            var type = AppDomain.CurrentDomain.GetAssemblies().Select(a => a.GetType("CavesOfOoo.Scenarios.Custom.GameAuditSplitIdentityBench")).FirstOrDefault(t => t != null);
            Assert.NotNull(type, "Split identity audit must be launchable."); return (IScenario)Activator.CreateInstance(type);
        }
        ...
        [Test] public void ArenaStartsWithActualUnspentComponentsAndPaidRecipeResources()
        {
            ...
            CollectionAssert.AreEqual(new[] { "Dagger", "SteelBladeComponent", "OakHaftComponent", "LeatherBindingComponent", "IronSpikeComponent", "GlimmerBrine", "SparkRoot" }, inv.Objects.Select(i => i.BlueprintName));
            CollectionAssert.AreEqual(new[] { 2, 2, 2, 2, 1, 1, 1 }, inv.Objects.Select(i => i.GetPart<StackerPart>().StackCount));
        ...
        [TestCase(false)] [TestCase(true)] public void AuditObservationPreservesDiagnostics(bool enabled)
        {
            ...
            Assert.AreEqual(enabled, Diag.IsChannelEnabled("scenario")); Assert.AreEqual(1, DiagQuery.Count(new DiagQuery.Filter { Kind = "SplitIdentityNativeAudit" }).Count);
        }
    }
}
