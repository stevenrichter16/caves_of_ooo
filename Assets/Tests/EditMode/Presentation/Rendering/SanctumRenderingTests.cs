...
{
    public sealed class SanctumRenderingTests
    {
        [TestCase(false)] [TestCase(true)]
        public void StateReactiveNativePopulationAlsoHasVoxelCoverage(bool urquActive)
        {
            var prior=NarrativeStatePart.Current;
            try
            {
                NarrativeStatePart.Current=new NarrativeStatePart();
                NarrativeStatePart.Current.SetFact("UrquActive",urquActive?1:0);
                EveryVisibleNativeOwnerAcrossSixChunksHasAReadOnlyModel(64);
            }
            finally{NarrativeStatePart.Current=prior;}
        }

        [TestCase("Overworld.5.4.0")] [TestCase("Overworld.5.4.1")] [TestCase("Overworld.5.4.2")]
        [TestCase("Overworld.2.4.0")] [TestCase("Overworld.2.4.1")] [TestCase("Overworld.2.4.2")]
        public void TheSixNativeChunksBindVoxelPresentationWithoutUnmappedSources(string id)
        {
        ...
        [TestCase("Overworld.5.4.3")] [TestCase("Overworld.2.4.3")]
        [TestCase("Overworld.4.6.3")] [TestCase("Overworld.12.3.2")]
        [TestCase("Overworld.05.4.2")] [TestCase("Overworld.2.04.2")]
        ...
        public void EveryVisibleNativeOwnerAcrossSixChunksHasAReadOnlyModel(int seed)
        {
            ...
            Assert.AreEqual(6,count);Assert.IsEmpty(missing,string.Join("\n",missing));
        }

        [TestCase("Overworld.5.4.2","SubstrateVault","cathedral-vault-")]
        [TestCase("Overworld.5.4.2","ChoirNode","cathedral-node-")]
        [TestCase("Overworld.5.4.2","EncasedElder","cathedral-elder-")]
        [TestCase("Overworld.5.4.2","ChoirTendril","cathedral-tendril-")]
        [TestCase("Overworld.2.4.2","LibraryTepuiboneWall","stillleaf-tepuibone-")]
        [TestCase("Overworld.2.4.2","LibraryMemoryMarbleWall","stillleaf-marble-")]
        [TestCase("Overworld.2.4.2","LibraryChoirIronWall","stillleaf-iron-")]
        [TestCase("Overworld.2.4.2","SealedArchiveShelf","stillleaf-shelf-")]
        public void DefiningOwnersHaveDistinctCurrentNativeModels(string id,string blueprint,string prefix)
        {
            ...
            Assert.AreSame(owner,recipe.Owner);
            Assert.AreEqual(owner.HasTag("Creature"),recipe.Transient);
        ...
        { foreach(var xy in new[]{"5.4","2.4"})for(int d=0;d<3;d++)yield return "Overworld."+xy+"."+d; }
    }
}
