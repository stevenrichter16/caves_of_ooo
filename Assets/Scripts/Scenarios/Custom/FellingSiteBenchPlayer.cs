...
namespace CavesOfOoo.Scenarios.Custom
{
    ...
    /// capture. The capture uses actual input-driven player turns.</summary>
    public sealed class FellingSiteBenchPlayer : MonoBehaviour
    {
        ...
        private FellingSiteBench _bench;
        private double _created, _started;
        private int _frames, _startTicks, _stage;
        private readonly string[] _names = { "COO.Turns.ProcessUntilPlayerTurn", "COO.Turns.FellingExposure", "COO.Input.Update", "COO.ZoneRenderer.LateUpdate", "GC Allocated In Frame" };
        ...
        public void Initialize(ScenarioContext ctx, FellingSiteBench bench)
        {
        ...
        private void Finish()
        {
            ...
            bool nativeWait = ticks >= 1000 && FellingSiteBench.Confused(_ctx.PlayerEntity)
                && _ctx.PlayerEntity.GetStatValue("DV") == _bench.ProfileDV - 2
                && _ctx.Zone.GetEntityPosition(_ctx.PlayerEntity) == (FellingSiteBuilder.SeventhX, FellingSiteBuilder.SeventhY);
            _bench.Check("native_waits_on_point", nativeWait); if (!nativeWait) Failures++;
            ...
            Directory.CreateDirectory(directory); ReportPath = Path.Combine(directory, "W65-live-profile.json");
            File.WriteAllText(Path.Combine(directory, "W65-live-frames.csv"), csv.ToString());
            File.WriteAllText(ReportPath, JsonUtility.ToJson(report, true));
            Diag.Record("scenario", "FellingSiteProfile", payload: new { runId = _bench.RunId, ticks, frames = _frames, failures = Failures });
            Debug.Log("[FellingSiteBench] " + JsonUtility.ToJson(report));
        }
