...
{
    // Optional explicit native profile; no automatic runtime/Editor hook.
    public sealed partial class SpawnRing3DNativeAudit
    {
        const int ProfileCapacity=200000;
        const double ProfilePhaseSeconds=5;
        ...
        IEnumerator RunRingProfileIfRequested()
        {
            var args=Environment.GetCommandLineArgs();
            for(int i=0;i<args.Length-1;i++)if(args[i]=="-spawnRing3dProfile")profileMode=args[i+1];
            if(string.IsNullOrEmpty(profileMode))yield break;
            Require(profileMode=="full"||profileMode=="low","Profile mode must be explicit full or low.");
            Require(Screen.width==1920&&Screen.height==1080&&State()=="Normal"&&Alive(),"Actual healthy1080p native profile precondition.");
            AllocateRingProfile();
            bool oldLow=Village3DSettings.LowDetail;
            try
            {
                Village3DSettings.LowDetail=profileMode=="low";
                for(int y=5;y<=7;y++)for(int x=2;x<=4;x++)
                {
                    if(x==3&&y==6)continue;
                    Zone zone=input.ZoneManager.GetZone(Id(x,y));
                    var lane=ProfileLane(zone);
                    // Existing real native zone is cached. Only the first observed
                    // presenter bind is timed here; shared assets may already be warm.
                    var binding=BeginProfilePhase(zone.ZoneID,"first_binding");
                    double syncStart=ProfileNow;FixturePlace(zone,lane.a);binding.bindCallMilliseconds=(ProfileNow-syncStart)*1000;
                    yield return WaitForRing();
                    EndProfilePhase();
                    // Exclude settling and screenshot overhead from steady samples.
                    yield return null;yield return null;
                    BeginProfilePhase(zone.ZoneID,"idle");
                    double end=ProfileNow+ProfilePhaseSeconds;
                    while(ProfileNow<end)yield return null;
                    EndProfilePhase();
                    var walk=BeginProfilePhase(zone.ZoneID,"walk");end=ProfileNow+ProfilePhaseSeconds;
                    var preferred=lane.b;var excluded=(-1,-1);int consecutiveUnmoved=0;
                    while(ProfileNow<end)
                    {
                        Require(Alive()&&State()=="Normal"&&ReferenceEquals(input.CurrentZone,zone),"Native walk lost healthy current-zone state; no AI/health workaround.");
                        var from=Position();
                        ...
                        Require(TryProfileWalkTarget(zone,input.PlayerEntity,from,preferred,excluded,out var target),
                            "No currently passable native cardinal neighbor; adaptive walking remains incomplete without clearing actors or terrain.");
                        ...
                        if(step.pass)
                        {nativeSteps++;walk.successfulSteps++;preferred=from;excluded=(-1,-1);consecutiveUnmoved=0;}
                        else
                        {
                            // Input may race a wandering actor or a native movement
                            // gate. Keep its exact failed receipt; never count it as a step.
                            Require(healthy&&after==from&&step.afterTick>=step.beforeTick,"Ordinary profile input displaced player or lost native healthy state.");
                            walk.unmovedInputs++;excluded=target;consecutiveUnmoved++;
                            Require(consecutiveUnmoved<4,"Four consecutive ordinary inputs did not move; workload is incomplete.");
                        }
                        // Explicit paced turn workload. Retain rendered/AI aftermath
                        // between commands; do not alter game speed, health or turns.
                        while(ProfileNow<nextInput&&ProfileNow<end)yield return null;
                    }
                    EndProfilePhase();
                    Require(walk.successfulSteps>=2,"Each zone needs multiple real native movement/turn controls.");
                    yield return Capture("profile-"+profileMode+"-"+zone.ZoneID);
                }
                double steady=profilePhases.Where(p=>p.kind!="first_binding").Sum(p=>p.seconds);
                Require(!profileOverflow&&profileBadResolutionFrames==0&&profilePhases.Count==24
                    &&profilePhases.Where(p=>p.kind!="first_binding").All(p=>p.seconds>=5&&p.frames>1)
                    &&steady>=80&&steady<=90,"Eight-zone steady capture must retain80–90 actual sampled seconds without overflow/wrong resolution.");
                Require(profilePhases.Where(p=>p.kind=="idle").All(p=>p.startTick==p.endTick&&p.startX==p.endX&&p.startY==p.endY),"Idle is a no-input control; live world must not silently move/advance it.");
                profileComplete=true;
                Check("native_ring_80second_profile_complete",true,"Eight5s idle +5s ordinary walking segments; first binding and capture gaps are separate.");
            }
            finally
            {
                if(profileCurrentPhase>=0)EndProfilePhase(false);
                DisposeRingProfile();profileEnd=ProfileNow;Village3DSettings.LowDetail=oldLow;
            }
        }
        ...
        static bool ProfileWalkCell(Zone zone,Entity player,(int x,int y) at)
        {
            ...
            return cell!=null&&!cell.BlocksMovement(player)
                &&!cell.Objects.Any(e=>!ReferenceEquals(e,player)&&e.HasTag("Creature"));
        }
        ...
        ProfileReport FinishRingProfile()
        {
            ...
            if(!valid){profileFailure=fatal??"Incomplete workload, required counters or frame coverage.";Check("native_ring_profile_valid",false,profileFailure);}
            profileResult=new ProfileReport{runId=RunId,saveRoot=saveRoot,mode=profileMode,valid=valid,complete=profileComplete,overflow=profileOverflow,failure=profileFailure,
                ...
                bounds="Actual native1920x1080 GameView and private seeded N game. Eight real cached native zones; fixture placement only between phases, then5s no-input plus5s ordinary keyboard walking per zone, with a minimum0.8s between issued commands. AI/health/factions/FOV/native turn behavior unchanged. World generation seed is fixed but independent trader/ambient/combat RNGs remain native and are not all seeded by this harness. No FullReveal, no speed/vsync or permanent preference changes. Preferred cells can be blocked by native actors: each rejected target is retained without issuing input, then another currently passable cardinal neighbor is chosen. Actual input without movement is retained as a failed attempt and never counted as a step; four consecutive such inputs, no alternative route, native displacement, modal state or death make the run incomplete. No obstacles or mechanics are suppressed. Steady80–90s is a sum of sustained segments; bind/settle/screenshots are excluded gaps, not one uninterrupted80s route. first_binding includes real fixture membership transfer, normal binder/autosave/readiness and its settling wait; bindCallMilliseconds is the synchronous placement+binder duration, not pure GPU or loading CPU. Zones/shared assets can already be cached; no process-cold claim. Recorder values are latest available completed samples, which can lag the logged Unity frame/phase boundary; GPU latency may differ. Counter units are taken from Unity descriptors. Missing counters are NA, not zero. Main/render/GPU/GC and draw counts include Editor/UI/observer; they do not isolate only ring geometry or establish whole-zone visibility. P95/p99 use all available per-phase samples, nearest rank, with missing/positive counts explicit. Normal native FOV and cropped viewport mean whole-zone source triangles are not a measured per-frame draw count. Low-detail changes RT/shadows only; no mesh-LOD claim. Adaptive local cardinal walking prefers returning to the previous cell but reroutes around current native occupancy; route/attempt differences are explicit. It is a local redraw/turn workload, not broad traversal/combat stress. This single Editor run cannot establish stable60fps, production-build budget, speedup or full-vs-low causality."};
