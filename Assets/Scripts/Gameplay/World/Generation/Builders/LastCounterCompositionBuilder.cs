...
{
    /// <summary>Fresh native frontier construction, staged before committing
    /// owners, interior markers or reservations. Never reconstructs a live graph.</summary>
    public sealed class LastCounterCompositionBuilder:IZoneBuilder
    {
        public string Name=>"LastCounterComposition";public int Priority=>1000;
        public LastCounterCompositionPlan Plan{get;private set;}
        public Zone RealizedZone{get;private set;}
        internal bool ProfileRealized;
        private readonly int seed;
        public LastCounterCompositionBuilder(int seed){this.seed=seed;}
        public bool BuildZone(Zone zone,EntityFactory factory,Random rng)
        {
            ...
            if(!LastCounterCompositionPlan.IsSupportedZone(zone.ZoneID))return Reject(zone,"outside-last-counter");
            if(zone.EntityCount!=0||(RealizedZone!=null&&!ReferenceEquals(RealizedZone,zone)))return Reject(zone,"nonempty-or-foreign-reuse");
            var plan=LastCounterCompositionPlan.Create(zone.ZoneID,seed);var required=new HashSet<string>();
            ...
            Diag.Record("worldgen","LastCounterCompositionPlanned",payload:new{zoneId=zone.ZoneID,seed,rooms=plan.Rooms.Count,formation=plan.FormationName});return true;
        }
        ...
        internal static bool Valid(Entity e,string bp)
        {
            ...
            bool actor=bp=="SaccharineEnvoy";
            bool solid=actor||bp=="SandstoneWall"||bp=="Chest"||bp=="Crate"||bp=="LastCounterSign";
            ...
            if(actor)return e.HasTag("Creature")&&e.HasPart<BrainPart>()&&e.HasPart<InventoryPart>()&&e.GetStatValue("Hitpoints")>0
                &&e.GetPart<ConversationPart>()?.ConversationID=="SaccharineEnvoy_1"&&e.GetTag("Faction")=="SaccharineConcord"
                &&e.GetPart<TraderPart>()?.StockTable=="EnvoyStock"&&e.GetPart<TraderPart>().Drams>0;
            if(e.HasPart<BrainPart>()||e.HasTag("Creature")||e.HasPart<LiquidPoolPart>()||e.HasPart<TileStateSourcePart>())return false;
            switch(bp)
            {
                case "Floor":case "StoneFloor":case "Rubble":return e.HasTag("Terrain");
                ...
                case "LastCounterSign":return e.HasTag("Furniture")&&!e.HasPart<DestructiblePart>()&&!e.HasPart<MaterialPart>();
        ...
        {
            switch(bp){case "Floor":case "StoneFloor":return ".";case "SandstoneWall":return "#";
                case "Bed":case "Chest":return "=";case "Chair":return "h";case "Crate":return "0";
        ...
        {Diag.Record("worldgen","LastCounterCompositionRejected",payload:new{zoneId=z?.ZoneID,seed,reason});return false;}
    }

    /// <summary>Place the four current native frontier owners once. All four
    /// validate before any of them becomes visible in the realized zone.</summary>
    public sealed class LastCounterProfileBuilder:IZoneBuilder
    {
        public string Name=>"LastCounterProfile";public int Priority=>3860;
        private readonly LastCounterCompositionBuilder terrain;
        public LastCounterProfileBuilder(LastCounterCompositionBuilder terrain){this.terrain=terrain;}
        public bool BuildZone(Zone zone,EntityFactory factory,Random rng)
        {
            ...
            if(!LastCounterCompositionBuilder.StockDependenciesValid(factory))return Reject(zone,"invalid-profile-stock");
            var staged=new List<(Entity e,int x,int y)>();
            foreach(var p in terrain.Plan.Profile)
            {
                ...
                var e=LastCounterCompositionBuilder.Create(factory,p.Blueprint);if(!LastCounterCompositionBuilder.Valid(e,p.Blueprint))return Reject(zone,"invalid-profile:"+p.Blueprint);
                ...
                if(LootTableRegistry.IsInitialized)
                {
                    if(p.Blueprint=="Chest")
                    {
                        ...
                        {var item=LastCounterCompositionBuilder.Create(factory,bp);if(item==null||!e.GetPart<ContainerPart>().AddItem(item))return Reject(zone,"chest-stock-refused");}
                    }
                    else if(p.Blueprint=="SaccharineEnvoy"&&e.GetPart<InventoryPart>().Objects.Count==0)
        ...
        {Diag.Record("worldgen","LastCounterProfileRejected",payload:new{zoneId=z?.ZoneID,reason});return false;}
    }
    ...
    public sealed class LastCounterArrivalReservationBuilder:IZoneBuilder
    {
        public string Name=>"LastCounterArrivalReservation";public int Priority=>3870;
        private readonly LastCounterCompositionBuilder terrain;
        public LastCounterArrivalReservationBuilder(LastCounterCompositionBuilder terrain){this.terrain=terrain;}
        public bool BuildZone(Zone zone,EntityFactory factory,Random rng)
        {
            if(zone==null||factory?.Blueprints==null||rng==null||terrain?.Plan==null||!ReferenceEquals(terrain.RealizedZone,zone))
            {Diag.Record("worldgen","LastCounterArrivalsRejected",payload:new{zoneId=zone?.ZoneID,reason="wrong-owner-or-dependency"});return false;}
            ...
            Diag.Record("worldgen","LastCounterArrivalsReserved",payload:new{zoneId=zone.ZoneID,stairs});return true;
        }
    }
}
