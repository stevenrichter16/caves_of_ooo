...
{
    /// <summary>Fresh native excavation construction, staged before committing
    /// owners, interior markers or reservations. Never reconstructs a live graph.</summary>
    public sealed class DrownedLedgerCompositionBuilder:IZoneBuilder
    {
        public string Name=>"DrownedLedgerComposition";public int Priority=>1000;
        public DrownedLedgerCompositionPlan Plan{get;private set;}
        public Zone RealizedZone{get;private set;}
        internal bool ProfileRealized;
        private readonly int seed;
        public DrownedLedgerCompositionBuilder(int seed){this.seed=seed;}
        public bool BuildZone(Zone zone,EntityFactory factory,Random rng)
        {
            ...
            if(!DrownedLedgerCompositionPlan.IsSupportedZone(zone.ZoneID))return Reject(zone,"outside-ledger");
            if(zone.EntityCount!=0||(RealizedZone!=null&&!ReferenceEquals(RealizedZone,zone)))return Reject(zone,"nonempty-or-foreign-reuse");
            var plan=DrownedLedgerCompositionPlan.Create(zone.ZoneID,seed);var required=new HashSet<string>();
            ...
            Diag.Record("worldgen","DrownedLedgerCompositionPlanned",payload:new{zoneId=zone.ZoneID,seed,rooms=plan.Rooms.Count,bays=plan.Bays.Count});return true;
        }
        ...
        public bool CanPlaceCaveEntrance(Zone zone,Cell cell)
        {
            ...
            for(int dy=-1;dy<=1;dy++)for(int dx=-1;dx<=1;dx++)
            {
                ...
                if(c==null||c.IsInterior||Plan.IsWet(x,y)||Plan.IsApproach(x,y)||zone.GenReservedCells.Contains((x,y))||c.BlocksMovement())return false;
        ...
        internal static bool Valid(Entity e,string bp)
        {
            ...
            bool actor=bp=="RecensionScribe"||bp=="CurationSorter";
            bool solid=actor||bp=="TentWall"||bp=="SandstoneWall"||bp=="PeatBank"||bp=="ReadingTable"||bp=="Crate"||bp=="DeadTree";
            if(physics.Solid!=solid||physics.Takeable||(!solid&&e.HasTag("Solid")))return false;
            if(actor)return e.HasTag("Creature")&&e.HasPart<BrainPart>()&&e.HasPart<InventoryPart>()&&e.GetStatValue("Hitpoints")>0
                &&e.GetPart<ConversationPart>()?.ConversationID==bp+"_1"&&e.HasPart<AISelfPreservationPart>()&&!e.HasPart<TraderPart>()
                &&e.GetTag("Faction")== (bp=="RecensionScribe"?"Palimpsest":"PaleCuration");
            if(e.HasTag("Creature")||e.HasPart<BrainPart>())return false;
            if(bp=="WaterPuddle")return e.GetPart<LiquidPoolPart>()?.LiquidId=="water"&&e.GetPart<LiquidPoolPart>().Volume>0
                &&e.GetPart<MaterialPart>()?.MaterialID=="Water"&&e.HasPart<ThermalPart>()&&!e.HasPart<TileStateSourcePart>();
            if(e.HasPart<LiquidPoolPart>()||e.HasPart<TileStateSourcePart>())return false;
            switch(bp)
            {
                case "Floor":case "StoneFloor":return e.HasTag("Terrain");
                case "TentWall":return e.HasTag("Wall")&&e.GetPart<MaterialPart>()?.MaterialID=="Cloth"&&e.HasPart<ThermalPart>()&&Healthy(e);
                case "SandstoneWall":return e.HasTag("Wall")&&e.GetPart<MaterialPart>()?.MaterialID=="Stone"&&Healthy(e);
                case "PeatBank":return e.GetPart<MaterialPart>()?.MaterialID=="Peat"&&e.HasPart<ThermalPart>()&&Healthy(e);
                case "DeadTree":return e.GetPart<MaterialPart>()?.MaterialID=="Wood"&&e.HasPart<ThermalPart>()&&e.HasTag("Vegetation")&&Healthy(e);
                ...
                case "Bed":return e.HasPart<BedPart>();case "Chair":return e.HasPart<ChairPart>();case "Reeds":return e.HasTag("Vegetation");
                case "PreFellingBody":return physics.Weight==80&&!e.HasPart<ContainerPart>()&&!e.HasPart<DestructiblePart>()&&!e.HasPart<ConversationPart>();
                case "ReadingTable":return e.HasTag("Furniture")&&!e.HasPart<ContainerPart>()&&!e.HasPart<DestructiblePart>();
                case "SurveyStake":return !e.HasPart<DestructiblePart>();default:return false;
            }
        }
        ...
        {
            switch(bp){case "Floor":case "StoneFloor":return ".";case "TentWall":case "SandstoneWall":case "PeatBank":return "#";
                case "WaterPuddle":return "~";case "Duckboard":case "Bed":case "ReadingTable":return "=";case "Chair":return "h";
                case "Crate":return "0";case "Reeds":case "SurveyStake":return "|";case "PreFellingBody":return "&";case "DeadTree":return "T";
                case "RecensionScribe":case "CurationSorter":return "@";default:return null;}
        }
        private bool Reject(Zone z,string reason)
        {Diag.Record("worldgen","DrownedLedgerCompositionRejected",payload:new{zoneId=z?.ZoneID,seed,reason});return false;}
    }

    /// <summary>Place the nine current native expedition owners once. All nine
    /// validate before any of them becomes visible in the realized zone.</summary>
    public sealed class DrownedLedgerProfileBuilder:IZoneBuilder
    {
        public string Name=>"DrownedLedgerProfile";public int Priority=>3860;
        private readonly DrownedLedgerCompositionBuilder terrain;
        public DrownedLedgerProfileBuilder(DrownedLedgerCompositionBuilder terrain){this.terrain=terrain;}
        public bool BuildZone(Zone zone,EntityFactory factory,Random rng)
        {
            ...
            foreach(var p in terrain.Plan.Profile)
            {
                ...
                var e=DrownedLedgerCompositionBuilder.Create(factory,p.Blueprint);if(!DrownedLedgerCompositionBuilder.Valid(e,p.Blueprint))return Reject(zone,"invalid-profile:"+p.Blueprint);
            ...
            terrain.ProfileRealized=true;Diag.Record("worldgen","DrownedLedgerProfilePlaced",payload:new{zoneId=zone.ZoneID,owners=added.Count});return true;
        }
        private static bool Reject(Zone z,string reason)
        {Diag.Record("worldgen","DrownedLedgerProfileRejected",payload:new{zoneId=z?.ZoneID,reason});return false;}
    }
    ...
    public sealed class DrownedLedgerArrivalReservationBuilder:IZoneBuilder
    {
        public string Name=>"DrownedLedgerArrivalReservation";public int Priority=>3870;
        private readonly DrownedLedgerCompositionBuilder terrain;
        public DrownedLedgerArrivalReservationBuilder(DrownedLedgerCompositionBuilder terrain){this.terrain=terrain;}
        public bool BuildZone(Zone zone,EntityFactory factory,Random rng)
        {
            if(zone==null||factory?.Blueprints==null||rng==null||terrain?.Plan==null||!ReferenceEquals(terrain.RealizedZone,zone))
            {Diag.Record("worldgen","DrownedLedgerArrivalsRejected",payload:new{zoneId=zone?.ZoneID,reason="wrong-owner-or-dependency"});return false;}
            int stairs=0;foreach(var e in zone.GetAllEntities())
            {
                ...
                {int x=c.X+dx,y=c.Y+dy;if(zone.InBounds(x,y)&&!terrain.Plan.IsWet(x,y)&&!zone.GetCell(x,y).BlocksMovement())zone.GenReservedCells.Add((x,y));}
            }
            Diag.Record("worldgen","DrownedLedgerArrivalsReserved",payload:new{zoneId=zone.ZoneID,stairs});return true;
        }
    }
}
