...
{
    /// <summary>Already-voxel Sumphold scenery. Entries describe art only: native
    /// entity membership owns visibility, interaction state, occupancy and damage.</summary>
    public sealed class SumpholdVoxelKitLibrary : ScriptableObject
    {
        public const string ResourcePath = "SumpholdVoxel3D/Library";
        ...
        public static SumpholdVoxelKitLibrary Load() => Resources.Load<SumpholdVoxelKitLibrary>(ResourcePath);
        ...
        public void Validate()
        {
            ...
            if (Entries == null || Entries.Length != Ids.Length)
                throw new InvalidOperationException("Sumphold voxel kit requires four variants of 6 families.");
            var ring = Resources.Load<SpawnRing3DLibrary>(SpawnRing3DLibrary.ResourcePath);
            if (ring == null || ring.WorldMaterial == null)
                throw new InvalidOperationException("Sumphold voxel kit requires the native ring palette material.");
            ...
            foreach (var e in Entries)
            {
                if (e == null || string.IsNullOrEmpty(e.Id) || e.Prefab == null || e.Mesh == null
                    ...
                    throw new InvalidOperationException("Invalid or duplicate Sumphold voxel entry.");
                var filter = e.Prefab.GetComponent<MeshFilter>();
                var renderer = e.Prefab.GetComponent<MeshRenderer>();
                var spec = e.Spec;
                bool isGround = e.Id.StartsWith("sumphold-ground-", StringComparison.Ordinal);
                ...
                if (filter == null || filter.sharedMesh != e.Mesh || renderer == null
                    ...
                    || spec.path != "Assets/Resources/SumpholdVoxel3D/" + e.Id + ".prefab"
                    ...
                    throw new InvalidOperationException("Sumphold voxel mesh, prefab or metadata mismatch: " + e.Id);
                candidates.Add(e.Id, e);
            }
            foreach (string id in Ids)
                if (!candidates.ContainsKey(id))
                    throw new InvalidOperationException("Missing Sumphold voxel variant: " + id);
            index = candidates;
        }

        public Entry Find(string id)
        {
            if (id == null || !id.StartsWith("sumphold-", StringComparison.Ordinal)) return null;
        ...
        public static string ModelId(string family, int variant)
        {
            ...
            switch (family)
            {
                ...
                case "hull": offset = 8; break;
                case "rolls": offset = 12; break;
                case "cutter": offset = 16; break;
                case "water": offset = 20; break;
                default: throw new ArgumentException("Unknown Sumphold voxel family.", nameof(family));
            }
        ...
        public static string Family(string blueprint)
        {
            switch (blueprint)
            {
                ...
                case "BoatFrame": return "hull";
                case "TollRolls": return "rolls";
                case "PeatCutter": return "cutter";
