...
{
    /// <summary>Already-voxel Gantry scenery. Entries describe art only: native
    /// entity membership owns visibility, interaction state, occupancy and damage.</summary>
    public sealed class GantryVoxelKitLibrary : ScriptableObject
    {
        public const string ResourcePath = "GantryVoxel3D/Library";
        ...
        public static GantryVoxelKitLibrary Load() => Resources.Load<GantryVoxelKitLibrary>(ResourcePath);
        ...
        public void Validate()
        {
            ...
            if (Entries == null || Entries.Length != Ids.Length)
                throw new InvalidOperationException("Gantry voxel kit requires four variants of 7 families.");
            var ring = Resources.Load<SpawnRing3DLibrary>(SpawnRing3DLibrary.ResourcePath);
            if (ring == null || ring.WorldMaterial == null)
                throw new InvalidOperationException("Gantry voxel kit requires the native ring palette material.");
            ...
            foreach (var e in Entries)
            {
                if (e == null || string.IsNullOrEmpty(e.Id) || e.Prefab == null || e.Mesh == null
                    ...
                    throw new InvalidOperationException("Invalid or duplicate Gantry voxel entry.");
                var filter = e.Prefab.GetComponent<MeshFilter>();
                var renderer = e.Prefab.GetComponent<MeshRenderer>();
                var spec = e.Spec;
                bool isGround = e.Id.StartsWith("gantry-ground-", StringComparison.Ordinal)||e.Id.StartsWith("gantry-path-", StringComparison.Ordinal);
                ...
                if (filter == null || filter.sharedMesh != e.Mesh || renderer == null
                    ...
                    || spec.path != "Assets/Resources/GantryVoxel3D/" + e.Id + ".prefab"
                    ...
                    throw new InvalidOperationException("Gantry voxel mesh, prefab or metadata mismatch: " + e.Id);
                candidates.Add(e.Id, e);
            }
            foreach (string id in Ids)
                if (!candidates.ContainsKey(id))
                    throw new InvalidOperationException("Missing Gantry voxel variant: " + id);
            index = candidates;
        }

        public Entry Find(string id)
        {
            if (id == null || !id.StartsWith("gantry-", StringComparison.Ordinal)) return null;
        ...
        public static string ModelId(string family, int variant)
        {
            ...
            switch (family)
            {
                ...
                case "path": offset = 4; break;
                case "wall": offset = 8; break;
                case "desk": offset = 12; break;
                case "counter": offset = 16; break;
                case "registrar": offset = 20; break;
                case "wayboard": offset = 24; break;
                default: throw new ArgumentException("Unknown Gantry voxel family.", nameof(family));
            }
        ...
        public static string Family(string blueprint)
        {
            switch (blueprint)
            {
                case "Floor":case "Grass":return "ground";case "RoadStone":return "path";
                case "GantryTimberWall":return "wall";case "GantryRegistryDesk":return "desk";
                case "GantryExchangeCounter":return "counter";case "GantryRegistrar":return "registrar";
                case "GantryWayboard":return "wayboard";default:return null;
            }
        }
    }
}
