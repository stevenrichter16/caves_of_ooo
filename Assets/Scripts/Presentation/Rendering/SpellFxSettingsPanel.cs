...
namespace CavesOfOoo.Rendering
{
    ...
    public sealed class SpellFxSettingsPanel : MonoBehaviour
    {
        ...
        {
            if (!IsOpen) return;
            _window = new Rect((Screen.width - 390) / 2f, (Screen.height - 360) / 2f, 390, 360);
            GUI.Window(GetInstanceID(), _window, Draw, "Spell effects · F10");
        }
        private void Draw(int id)
        {
            ...
            GUILayout.Label("Sound volume  " + Mathf.RoundToInt(SpellFxSettings.SoundVolume * 100) + "%");
            SpellFxSettings.SoundVolume = GUILayout.HorizontalSlider(SpellFxSettings.SoundVolume, 0f, 1f);
            GUILayout.Space(12);
