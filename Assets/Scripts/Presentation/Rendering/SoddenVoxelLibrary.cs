...
{
    /// <summary>Already-voxel Sodden scenery. Entries describe art only: native
    /// entity membership owns visibility, liquid contact, occupancy and damage.</summary>
    public sealed class SoddenVoxelLibrary : ScriptableObject
    {
        public const string ResourcePath = "SoddenVoxel3D/Library";
        ...
        public static SoddenVoxelLibrary Load() => Resources.Load<SoddenVoxelLibrary>(ResourcePath);
        ...
        public void Validate()
        {
            ...
            if (Entries == null || Entries.Length != Ids.Length)
                throw new InvalidOperationException("Sodden voxel kit requires four variants of six families.");
            var ring = Resources.Load<SpawnRing3DLibrary>(SpawnRing3DLibrary.ResourcePath);
            if (ring == null || ring.WorldMaterial == null)
                throw new InvalidOperationException("Sodden voxel kit requires the native ring palette material.");
            ...
            foreach (var e in Entries)
            {
                if (e == null || string.IsNullOrEmpty(e.Id) || e.Prefab == null || e.Mesh == null
                    ...
                    throw new InvalidOperationException("Invalid or duplicate Sodden voxel entry.");
                var filter = e.Prefab.GetComponent<MeshFilter>();
                var renderer = e.Prefab.GetComponent<MeshRenderer>();
                var spec = e.Spec;
                string expectedKind = e.Id.StartsWith("sodden-ground-", StringComparison.Ordinal) ? "ground" : "entity";
                if (filter == null || filter.sharedMesh != e.Mesh || renderer == null
                    ...
                    || spec.path != "Assets/Resources/SoddenVoxel3D/" + e.Id + ".prefab"
                    ...
                    throw new InvalidOperationException("Sodden voxel mesh, prefab or metadata mismatch: " + e.Id);
                candidates.Add(e.Id, e);
            }
            foreach (string id in Ids)
                if (!candidates.ContainsKey(id))
                    throw new InvalidOperationException("Missing Sodden voxel variant: " + id);
            index = candidates;
        }

        public Entry Find(string id)
        {
            if (id == null || !id.StartsWith("sodden-", StringComparison.Ordinal)) return null;
        ...
        public static string ModelId(string family, int variant)
        {
            ...
            switch (family)
            {
                ...
                case "mire": offset = 4; break;
                case "peat": offset = 8; break;
                case "snag": offset = 12; break;
                case "boards": offset = 16; break;
                case "body": offset = 20; break;
                default: throw new ArgumentException("Unknown Sodden voxel family.", nameof(family));
            }
        ...
        public static string Family(string blueprint)
        {
            switch (blueprint)
            {
                case "Grass": case "Floor": return "ground";
                case "MirePool": case "PeatBog": return "mire";
                case "PeatBank": return "peat";
                case "DeadTree": return "snag";
                case "Duckboard": return "boards";
                case "BogTakenBody": return "body";
                default: return null;
            }
        }
    }
