...
{
    /// <summary>Already-voxel Marrowstye scenery. Entries describe art only: native
    /// entity membership owns visibility, interaction state, occupancy and damage.</summary>
    public sealed class MarrowstyeVoxelKitLibrary : ScriptableObject
    {
        public const string ResourcePath = "MarrowstyeVoxel3D/Library";
        ...
        public static MarrowstyeVoxelKitLibrary Load() => Resources.Load<MarrowstyeVoxelKitLibrary>(ResourcePath);
        ...
        public void Validate()
        {
            ...
            if (Entries == null || Entries.Length != Ids.Length)
                throw new InvalidOperationException("Marrowstye voxel kit requires four variants of 6 families.");
            var ring = Resources.Load<SpawnRing3DLibrary>(SpawnRing3DLibrary.ResourcePath);
            if (ring == null || ring.WorldMaterial == null)
                throw new InvalidOperationException("Marrowstye voxel kit requires the native ring palette material.");
            ...
            foreach (var e in Entries)
            {
                if (e == null || string.IsNullOrEmpty(e.Id) || e.Prefab == null || e.Mesh == null
                    ...
                    throw new InvalidOperationException("Invalid or duplicate Marrowstye voxel entry.");
                var filter = e.Prefab.GetComponent<MeshFilter>();
                var renderer = e.Prefab.GetComponent<MeshRenderer>();
                var spec = e.Spec;
                bool isGround = e.Id.StartsWith("marrowstye-ground-", StringComparison.Ordinal)
                    || e.Id.StartsWith("marrowstye-path-", StringComparison.Ordinal);
                ...
                if (filter == null || filter.sharedMesh != e.Mesh || renderer == null
                    ...
                    || spec.path != "Assets/Resources/MarrowstyeVoxel3D/" + e.Id + ".prefab"
                    ...
                    throw new InvalidOperationException("Marrowstye voxel mesh, prefab or metadata mismatch: " + e.Id);
                candidates.Add(e.Id, e);
            }
            foreach (string id in Ids)
                if (!candidates.ContainsKey(id))
                    throw new InvalidOperationException("Missing Marrowstye voxel variant: " + id);
            index = candidates;
        }

        public Entry Find(string id)
        {
            if (id == null || !id.StartsWith("marrowstye-", StringComparison.Ordinal)) return null;
        ...
        private static string[] MakeIds()
        {
            ...
            foreach (string family in Families)
                for (int variant = 0; variant < VariantCount; variant++)
                    ids[n++] = "marrowstye-" + family + "-" + variant;
            return ids;
        }
        ...
        public static string ModelId(string family, int variant)
        {
            ...
            switch (family)
            {
                ...
                case "coffer": offset = 8; break;
                case "cured": offset = 12; break;
                case "clerk": offset = 16; break;
                case "path": offset = 20; break;
                default: throw new ArgumentException("Unknown Marrowstye voxel family.", nameof(family));
            }
        ...
        public static string Family(string blueprint)
        {
            switch (blueprint)
            {
                ...
                case "StoneCoffer": return "coffer";
                case "SaltCuredBody": return "cured";
                case "FilerClerk": return "clerk";
                default: return null;
            }
        }
    }
