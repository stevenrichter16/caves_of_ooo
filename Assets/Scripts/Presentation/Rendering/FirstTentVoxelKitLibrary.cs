...
{
    /// <summary>Already-voxel FirstTent scenery. Entries describe art only: native
    /// entity membership owns visibility, interaction state, occupancy and damage.</summary>
    public sealed class FirstTentVoxelKitLibrary : ScriptableObject
    {
        public const string ResourcePath = "FirstTentVoxel3D/Library";
        ...
        public static FirstTentVoxelKitLibrary Load() => Resources.Load<FirstTentVoxelKitLibrary>(ResourcePath);
        ...
        public void Validate()
        {
            ...
            if (Entries == null || Entries.Length != Ids.Length)
                throw new InvalidOperationException("FirstTent voxel kit requires four variants of 6 families.");
            var ring = Resources.Load<SpawnRing3DLibrary>(SpawnRing3DLibrary.ResourcePath);
            if (ring == null || ring.WorldMaterial == null)
                throw new InvalidOperationException("FirstTent voxel kit requires the native ring palette material.");
            ...
            foreach (var e in Entries)
            {
                if (e == null || string.IsNullOrEmpty(e.Id) || e.Prefab == null || e.Mesh == null
                    ...
                    throw new InvalidOperationException("Invalid or duplicate FirstTent voxel entry.");
                var filter = e.Prefab.GetComponent<MeshFilter>();
                var renderer = e.Prefab.GetComponent<MeshRenderer>();
                var spec = e.Spec;
                bool isGround = e.Id.StartsWith("firsttent-ground-", StringComparison.Ordinal)
                    || e.Id.StartsWith("firsttent-path-", StringComparison.Ordinal);
                ...
                if (filter == null || filter.sharedMesh != e.Mesh || renderer == null
                    ...
                    || spec.path != "Assets/Resources/FirstTentVoxel3D/" + e.Id + ".prefab"
                    ...
                    throw new InvalidOperationException("FirstTent voxel mesh, prefab or metadata mismatch: " + e.Id);
                candidates.Add(e.Id, e);
            }
            foreach (string id in Ids)
                if (!candidates.ContainsKey(id))
                    throw new InvalidOperationException("Missing FirstTent voxel variant: " + id);
            index = candidates;
        }

        public Entry Find(string id)
        {
            if (id == null || !id.StartsWith("firsttent-", StringComparison.Ordinal)) return null;
        ...
        private static string[] MakeIds()
        {
            ...
            foreach (string family in Families)
                for (int variant = 0; variant < VariantCount; variant++)
                    ids[n++] = "firsttent-" + family + "-" + variant;
            return ids;
        }
        ...
        public static string ModelId(string family, int variant)
        {
            ...
            switch (family)
            {
                ...
                case "corner": offset = 12; break;
                case "cloth": offset = 16; break;
                case "host": offset = 20; break;
                default: throw new ArgumentException("Unknown FirstTent voxel family.", nameof(family));
            }
