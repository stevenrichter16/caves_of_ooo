...
{
    /// <summary>Already-voxel Ginmere scenery. Entries describe art only: native
    /// entity membership owns visibility, liquid contact, occupancy and damage.</summary>
    public sealed class GinmereVoxelLibrary : ScriptableObject
    {
        public const string ResourcePath = "GinmereVoxel3D/Library";
        ...
        public static GinmereVoxelLibrary Load() => Resources.Load<GinmereVoxelLibrary>(ResourcePath);
        ...
        public void Validate()
        {
            ...
            if (Entries == null || Entries.Length != Ids.Length)
                throw new InvalidOperationException("Ginmere voxel kit requires four variants of sixteen families.");
            var ring = Resources.Load<SpawnRing3DLibrary>(SpawnRing3DLibrary.ResourcePath);
            if (ring == null || ring.WorldMaterial == null)
                throw new InvalidOperationException("Ginmere voxel kit requires the native ring palette material.");
            ...
            foreach (var e in Entries)
            {
                if (e == null || string.IsNullOrEmpty(e.Id) || e.Prefab == null || e.Mesh == null
                    ...
                    throw new InvalidOperationException("Invalid or duplicate Ginmere voxel entry.");
                var filter = e.Prefab.GetComponent<MeshFilter>();
                var renderer = e.Prefab.GetComponent<MeshRenderer>();
                var spec = e.Spec;
                bool isGround = e.Id.StartsWith("ginmere-ground-", StringComparison.Ordinal);
                ...
                if (filter == null || filter.sharedMesh != e.Mesh || renderer == null
                    ...
                    || spec.path != "Assets/Resources/GinmereVoxel3D/" + e.Id + ".prefab"
                    ...
                    throw new InvalidOperationException("Ginmere voxel mesh, prefab or metadata mismatch: " + e.Id);
                candidates.Add(e.Id, e);
            }
            foreach (string id in Ids)
                if (!candidates.ContainsKey(id))
                    throw new InvalidOperationException("Missing Ginmere voxel variant: " + id);
            index = candidates;
        }

        public Entry Find(string id)
        {
            if (id == null || !id.StartsWith("ginmere-", StringComparison.Ordinal)) return null;
        ...
        public static string ModelId(string family, int variant)
        {
            ...
            switch (family)
            {
                ...
                case "cliff": offset = 4; break;
                case "rim": offset = 8; break;
                case "ledge": offset = 12; break;
                case "water": offset = 16; break;
                case "anchor": offset = 20; break;
                case "nest": offset = 24; break;
                case "gecko": offset = 28; break;
                case "frog": offset = 32; break;
                case "torch": offset = 36; break;
                case "meat": offset = 40; break;
                case "tonic": offset = 44; break;
                case "frost": offset = 48; break;
                case "ice": offset = 52; break;
                case "stalagmite": offset = 56; break;
                case "cache": offset = 60; break;
                default: throw new ArgumentException("Unknown Ginmere voxel family.", nameof(family));
            }
        ...
        public static string Family(string blueprint)
        {
            switch (blueprint)
            {
                ...
                case "SandstoneWall": return "cliff";
                case "SinkholeLip": return "rim";
                case "DescentLedge": return "ledge";
                case "MirePool": return "water";
                case "RopeAnchor": return "anchor";
                case "PricklebrowNest": return "nest";
                case "PrickleBrowGecko": return "gecko";
                case "GinFrog": return "frog";
                case "Torch": return "torch";
                case "DriedMeat": return "meat";
                case "HealingTonic": return "tonic";
                case "FrostVent": return "frost";
                case "IceSheet": return "ice";
                case "Stalagmite": return "stalagmite";
                case "BoneCache": return "cache";
                default: return null;
            }
        }
    }
