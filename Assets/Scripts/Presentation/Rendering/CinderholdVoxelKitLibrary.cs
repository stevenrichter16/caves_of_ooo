...
{
    /// <summary>Already-voxel Cinderhold scenery. Entries describe art only: native
    /// entity membership owns visibility, interaction state, occupancy and damage.</summary>
    public sealed class CinderholdVoxelKitLibrary : ScriptableObject
    {
        public const string ResourcePath = "CinderholdVoxel3D/Library";
        ...
        public static CinderholdVoxelKitLibrary Load() => Resources.Load<CinderholdVoxelKitLibrary>(ResourcePath);
        ...
        public void Validate()
        {
            ...
            if (Entries == null || Entries.Length != Ids.Length)
                throw new InvalidOperationException("Cinderhold voxel kit requires four variants of 10 families.");
            var ring = Resources.Load<SpawnRing3DLibrary>(SpawnRing3DLibrary.ResourcePath);
            if (ring == null || ring.WorldMaterial == null)
                throw new InvalidOperationException("Cinderhold voxel kit requires the native ring palette material.");
            ...
            foreach (var e in Entries)
            {
                if (e == null || string.IsNullOrEmpty(e.Id) || e.Prefab == null || e.Mesh == null
                    ...
                    throw new InvalidOperationException("Invalid or duplicate Cinderhold voxel entry.");
                var filter = e.Prefab.GetComponent<MeshFilter>();
                var renderer = e.Prefab.GetComponent<MeshRenderer>();
                var spec = e.Spec;
                bool isGround = e.Id.StartsWith("cinderhold-ground-", StringComparison.Ordinal)
                    || e.Id.StartsWith("cinderhold-path-", StringComparison.Ordinal);
                ...
                if (filter == null || filter.sharedMesh != e.Mesh || renderer == null
                    ...
                    || spec.path != "Assets/Resources/CinderholdVoxel3D/" + e.Id + ".prefab"
                    ...
                    throw new InvalidOperationException("Cinderhold voxel mesh, prefab or metadata mismatch: " + e.Id);
                candidates.Add(e.Id, e);
            }
            foreach (string id in Ids)
                if (!candidates.ContainsKey(id))
                    throw new InvalidOperationException("Missing Cinderhold voxel variant: " + id);
            index = candidates;
        }

        public Entry Find(string id)
        {
            if (id == null || !id.StartsWith("cinderhold-", StringComparison.Ordinal)) return null;
        ...
        private static string[] MakeIds()
        {
            ...
            foreach (string family in Families)
                for (int variant = 0; variant < VariantCount; variant++)
                    ids[n++] = "cinderhold-" + family + "-" + variant;
            return ids;
        }
        ...
        public static string ModelId(string family, int variant)
        {
            ...
            switch (family)
            {
                ...
                case "factor": offset = 8; break;
                case "notice": offset = 12; break;
                case "forge": offset = 16; break;
                case "anvil": offset = 20; break;
                case "smith": offset = 24; break;
                case "stall": offset = 28; break;
                case "token": offset = 32; break;
                case "path": offset = 36; break;
                default: throw new ArgumentException("Unknown Cinderhold voxel family.", nameof(family));
            }
        ...
        public static string Family(string blueprint)
        {
            switch (blueprint)
            {
                ...
                case "ConcordFactor": return "factor";
                case "CinderholdNoticeBoard": return "notice";
                case "TinkersForge": return "forge";
                case "SmithAnvil": return "anvil";
                case "Weaponsmith": return "smith";
                case "MarketStall": return "stall";
                case "RoadStone": return "path";
                // CrunchyLocket is dynamically authored; the renderer must first
                // verify its native CompleteObjectiveOnTaken contract. No alias.
                default: return null;
            }
        }
    }
