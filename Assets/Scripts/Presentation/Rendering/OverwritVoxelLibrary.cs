...
{
    /// <summary>Already-voxel Overwrit scenery. Entries describe art only: native
    /// entity membership owns visibility, liquid contact, occupancy and damage.</summary>
    public sealed class OverwritVoxelLibrary : ScriptableObject
    {
        public const string ResourcePath = "OverwritVoxel3D/Library";
        ...
        public static OverwritVoxelLibrary Load() => Resources.Load<OverwritVoxelLibrary>(ResourcePath);
        ...
        public void Validate()
        {
            ...
            if (Entries == null || Entries.Length != Ids.Length)
                throw new InvalidOperationException("Overwrit voxel kit requires four variants of four families.");
            var ring = Resources.Load<SpawnRing3DLibrary>(SpawnRing3DLibrary.ResourcePath);
            if (ring == null || ring.WorldMaterial == null)
                throw new InvalidOperationException("Overwrit voxel kit requires the native ring palette material.");
            ...
            foreach (var e in Entries)
            {
                if (e == null || string.IsNullOrEmpty(e.Id) || e.Prefab == null || e.Mesh == null
                    ...
                    throw new InvalidOperationException("Invalid or duplicate Overwrit voxel entry.");
                var filter = e.Prefab.GetComponent<MeshFilter>();
                var renderer = e.Prefab.GetComponent<MeshRenderer>();
                var spec = e.Spec;
                bool isGround = e.Id.StartsWith("overwrit-ground-", StringComparison.Ordinal);
                ...
                if (filter == null || filter.sharedMesh != e.Mesh || renderer == null
                    ...
                    || spec.path != "Assets/Resources/OverwritVoxel3D/" + e.Id + ".prefab"
                    ...
                    throw new InvalidOperationException("Overwrit voxel mesh, prefab or metadata mismatch: " + e.Id);
                candidates.Add(e.Id, e);
            }
            foreach (string id in Ids)
                if (!candidates.ContainsKey(id))
                    throw new InvalidOperationException("Missing Overwrit voxel variant: " + id);
            index = candidates;
        }

        public Entry Find(string id)
        {
            if (id == null || !id.StartsWith("overwrit-", StringComparison.Ordinal)) return null;
        ...
        public static string ModelId(string family, int variant)
        {
            ...
            switch (family)
            {
                ...
                case "growth": offset = 4; break;
                case "waymarker": offset = 8; break;
                case "bench": offset = 12; break;
                default: throw new ArgumentException("Unknown Overwrit voxel family.", nameof(family));
            }
        ...
        public static string Family(string blueprint)
        {
            switch (blueprint)
            {
                case "OverwritGround": return "ground";
                case "OverwritNewGrowth": return "growth";
                case "OverwritWaymarker": return "waymarker";
                case "OverwritPilgrimBench": return "bench";
                default: return null;
            }
        }
    }
