...
{
    /// <summary>Already-voxel Quillhold scenery. Entries describe art only: native
    /// entity membership owns visibility, interaction state, occupancy and damage.</summary>
    public sealed class QuillholdVoxelKitLibrary : ScriptableObject
    {
        public const string ResourcePath = "QuillholdVoxel3D/Library";
        ...
        public static QuillholdVoxelKitLibrary Load() => Resources.Load<QuillholdVoxelKitLibrary>(ResourcePath);
        ...
        public void Validate()
        {
            ...
            if (Entries == null || Entries.Length != Ids.Length)
                throw new InvalidOperationException("Quillhold voxel kit requires four variants of 7 families.");
            var ring = Resources.Load<SpawnRing3DLibrary>(SpawnRing3DLibrary.ResourcePath);
            if (ring == null || ring.WorldMaterial == null)
                throw new InvalidOperationException("Quillhold voxel kit requires the native ring palette material.");
            ...
            foreach (var e in Entries)
            {
                if (e == null || string.IsNullOrEmpty(e.Id) || e.Prefab == null || e.Mesh == null
                    ...
                    throw new InvalidOperationException("Invalid or duplicate Quillhold voxel entry.");
                var filter = e.Prefab.GetComponent<MeshFilter>();
                var renderer = e.Prefab.GetComponent<MeshRenderer>();
                var spec = e.Spec;
                bool isGround = e.Id.StartsWith("quillhold-ground-", StringComparison.Ordinal)
                    || e.Id.StartsWith("quillhold-path-", StringComparison.Ordinal);
                ...
                if (filter == null || filter.sharedMesh != e.Mesh || renderer == null
                    ...
                    || spec.path != "Assets/Resources/QuillholdVoxel3D/" + e.Id + ".prefab"
                    ...
                    throw new InvalidOperationException("Quillhold voxel mesh, prefab or metadata mismatch: " + e.Id);
                candidates.Add(e.Id, e);
            }
            foreach (string id in Ids)
                if (!candidates.ContainsKey(id))
                    throw new InvalidOperationException("Missing Quillhold voxel variant: " + id);
            index = candidates;
        }

        public Entry Find(string id)
        {
            if (id == null || !id.StartsWith("quillhold-", StringComparison.Ordinal)) return null;
        ...
        private static string[] MakeIds()
        {
            ...
            foreach (string family in Families)
                for (int variant = 0; variant < VariantCount; variant++)
                    ids[n++] = "quillhold-" + family + "-" + variant;
            return ids;
        }
        ...
        public static string ModelId(string family, int variant)
        {
            ...
            switch (family)
            {
                ...
                case "shelf": offset = 12; break;
                case "desk": offset = 16; break;
                case "scribe": offset = 20; break;
                case "table": offset = 24; break;
                default: throw new ArgumentException("Unknown Quillhold voxel family.", nameof(family));
            }
        ...
        public static string Family(string blueprint)
        {
            switch (blueprint)
            {
                ...
                case "QuillholdArchiveWall": return "wall";
                case "QuillholdArchiveShelf": return "shelf";
                case "QuillholdCopyDesk": return "desk";
                case "Scribe": return "scribe";
                case "QuillholdRefectoryTable": return "table";
                default: return null;
            }
        }
    }
