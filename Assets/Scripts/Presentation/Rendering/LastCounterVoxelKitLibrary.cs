...
{
    /// <summary>Already-voxel LastCounter scenery. Entries describe art only: native
    /// entity membership owns visibility, interaction state, occupancy and damage.</summary>
    public sealed class LastCounterVoxelKitLibrary : ScriptableObject
    {
        public const string ResourcePath = "LastCounterVoxel3D/Library";
        ...
        public static LastCounterVoxelKitLibrary Load() => Resources.Load<LastCounterVoxelKitLibrary>(ResourcePath);
        ...
        public void Validate()
        {
            ...
            if (Entries == null || Entries.Length != Ids.Length)
                throw new InvalidOperationException("LastCounter voxel kit requires four variants of 4 families.");
            var ring = Resources.Load<SpawnRing3DLibrary>(SpawnRing3DLibrary.ResourcePath);
            if (ring == null || ring.WorldMaterial == null)
                throw new InvalidOperationException("LastCounter voxel kit requires the native ring palette material.");
            ...
            foreach (var e in Entries)
            {
                if (e == null || string.IsNullOrEmpty(e.Id) || e.Prefab == null || e.Mesh == null
                    ...
                    throw new InvalidOperationException("Invalid or duplicate LastCounter voxel entry.");
                var filter = e.Prefab.GetComponent<MeshFilter>();
                var renderer = e.Prefab.GetComponent<MeshRenderer>();
                var spec = e.Spec;
                bool isGround = e.Id.StartsWith("lastcounter-ground-", StringComparison.Ordinal);
                ...
                if (filter == null || filter.sharedMesh != e.Mesh || renderer == null
                    ...
                    || spec.path != "Assets/Resources/LastCounterVoxel3D/" + e.Id + ".prefab"
                    ...
                    throw new InvalidOperationException("LastCounter voxel mesh, prefab or metadata mismatch: " + e.Id);
                candidates.Add(e.Id, e);
            }
            foreach (string id in Ids)
                if (!candidates.ContainsKey(id))
                    throw new InvalidOperationException("Missing LastCounter voxel variant: " + id);
            index = candidates;
        }

        public Entry Find(string id)
        {
            if (id == null || !id.StartsWith("lastcounter-", StringComparison.Ordinal)) return null;
        ...
        private static string[] MakeIds()
        {
            ...
            foreach (string family in Families)
                for (int variant = 0; variant < VariantCount; variant++)
                    ids[n++] = "lastcounter-" + family + "-" + variant;
            return ids;
        }
        ...
        public static string ModelId(string family, int variant)
        {
            ...
            switch (family)
            {
                ...
                case "sign": offset = 8; break;
                case "envoy": offset = 12; break;
                default: throw new ArgumentException("Unknown LastCounter voxel family.", nameof(family));
            }
        ...
        public static string Family(string blueprint)
        {
            switch (blueprint)
            {
                ...
                case "LastCounterSign": return "sign";
                case "SaccharineEnvoy": return "envoy";
                default: return null;
            }
        }
    }
