...
{
    /// <summary>Already-voxel Cathedral scenery. Entries describe art only: native
    /// entity membership owns visibility, interaction state, occupancy and damage.</summary>
    public sealed class CathedralVoxelLibrary : ScriptableObject
    {
        public const string ResourcePath = "CathedralVoxel3D/Library";
        ...
        public static CathedralVoxelLibrary Load() => Resources.Load<CathedralVoxelLibrary>(ResourcePath);
        ...
        public void Validate()
        {
            ...
            if (Entries == null || Entries.Length != Ids.Length)
                throw new InvalidOperationException("Cathedral voxel kit requires four variants of 5 families.");
            var ring = Resources.Load<SpawnRing3DLibrary>(SpawnRing3DLibrary.ResourcePath);
            if (ring == null || ring.WorldMaterial == null)
                throw new InvalidOperationException("Cathedral voxel kit requires the native ring palette material.");
            ...
            foreach (var e in Entries)
            {
                if (e == null || string.IsNullOrEmpty(e.Id) || e.Prefab == null || e.Mesh == null
                    ...
                    throw new InvalidOperationException("Invalid or duplicate Cathedral voxel entry.");
                var filter = e.Prefab.GetComponent<MeshFilter>();
                var renderer = e.Prefab.GetComponent<MeshRenderer>();
                var spec = e.Spec;
                bool isGround = e.Id.StartsWith("cathedral-ground-", StringComparison.Ordinal);
                ...
                if (filter == null || filter.sharedMesh != e.Mesh || renderer == null
                    ...
                    || spec.path != "Assets/Resources/CathedralVoxel3D/" + e.Id + ".prefab"
                    ...
                    throw new InvalidOperationException("Cathedral voxel mesh, prefab or metadata mismatch: " + e.Id);
                candidates.Add(e.Id, e);
            }
            foreach (string id in Ids)
                if (!candidates.ContainsKey(id))
                    throw new InvalidOperationException("Missing Cathedral voxel variant: " + id);
            index = candidates;
        }

        public Entry Find(string id)
        {
            if (id == null || !id.StartsWith("cathedral-", StringComparison.Ordinal)) return null;
        ...
        private static string[] MakeIds()
        {
            ...
            foreach (string family in Families)
                for (int variant = 0; variant < VariantCount; variant++)
                    ids[n++] = "cathedral-" + family + "-" + variant;
            return ids;
        }
        ...
        public static string ModelId(string family, int variant)
        {
            ...
            switch (family)
            {
                ...
                case "vault": offset = 4; break;
                case "node": offset = 8; break;
                case "elder": offset = 12; break;
                case "tendril": offset = 16; break;
                default: throw new ArgumentException("Unknown Cathedral voxel family.", nameof(family));
            }
        ...
        public static string Family(string blueprint)
        {
            switch (blueprint)
            {
                ...
                case "SubstrateVault": return "vault";
                case "ChoirNode": return "node";
                case "EncasedElder": return "elder";
                case "ChoirTendril": return "tendril";
                default: return null;
            }
        }
    }
