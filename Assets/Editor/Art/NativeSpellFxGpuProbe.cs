...
namespace CavesOfOoo.Editor
{
    ...
    public static class NativeSpellFxGpuProbe
    {
        ...
        [Serializable] public sealed class ColorUpload
        {
            ...
            public float initialEmission, retainedEmission, freshEmission, counterEmission;
        }
        [Serializable] public sealed class CaseRecord
        {
            ...
            public float maximumInitialVectorError, maximumRetainedVectorError, maximumFreshVectorError, maximumEmissionControlError;
        ...
        /// <summary>A fresh raw-vector color control, retaining the actual emission.
        /// Reusing a Color-typed property block can retain Unity conversion metadata;
        /// dropping emission would change a second variable in the color counter.</summary>
        internal static MaterialPropertyBlock CreateColorReferenceBlock(Vector4 linear, MaterialPropertyBlock original)
        {
            if (original == null) throw new ArgumentNullException(nameof(original));
            var result = new MaterialPropertyBlock();
            result.SetVector("_BaseColor", linear);
            result.SetFloat("_Emission", original.GetFloat("_Emission"));
            return result;
        }
        ...
        sealed class Probe : IDisposable
        {
            ...
            public void LinearColor(NativeSpellFxEntry entry, CaseRecord row)
            {
                ...
                foreach(var view in views)
                {
                    ...
                    var upload=new ColorUpload{piece=piece.Id,importedLinear=piece.Color,initialVector=block.GetVector(id),initialColor=block.GetColor(id),initialEmission=block.GetFloat("_Emission")};
                    block.SetVector(id,(Vector4)piece.Color);view.SetPropertyBlock(block);view.GetPropertyBlock(block);
                    upload.retainedVector=block.GetVector(id);upload.retainedColor=block.GetColor(id);upload.retainedEmission=block.GetFloat("_Emission");uploads.Add(upload);
                }
                ...
                {
                    views[i].GetPropertyBlock(block);
                    var fresh=CreateColorReferenceBlock(uploads[i].importedLinear,block);
                    views[i].SetPropertyBlock(fresh);views[i].GetPropertyBlock(block);
                    uploads[i].freshVector=block.GetVector(id);uploads[i].freshColor=block.GetColor(id);uploads[i].freshEmission=block.GetFloat("_Emission");
                }
                ...
                {
                    views[i].GetPropertyBlock(block);
                    var counter=CreateColorReferenceBlock((Vector4)((Color)uploads[i].importedLinear).linear,block);
                    views[i].SetPropertyBlock(counter);views[i].GetPropertyBlock(block);uploads[i].counterEmission=block.GetFloat("_Emission");
                }
                row.maximumEmissionControlError=uploads.Max(u=>Mathf.Max(Mathf.Abs(u.initialEmission-u.retainedEmission),Mathf.Abs(u.initialEmission-u.freshEmission),Mathf.Abs(u.initialEmission-u.counterEmission)));
                Require(Finite(row.maximumEmissionControlError)&&row.maximumEmissionControlError<=1e-6f,"Color controls must preserve actual emission exactly.");
