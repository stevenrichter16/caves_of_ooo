...
namespace CavesOfOoo.Editor
{
    ...
    /// Owns only SpawnRing art/library assets; never rewrites village/pipeline assets,
    /// modifies an open scene, deletes obsolete content or starts automatically.</summary>
    public static class SpawnRing3DAssetBuilder
    {
        const string Art="Assets/Art3D/SpawnRing";
        const string LibraryPath="Assets/Resources/SpawnRing3D/Library.asset";
        ...
        [MenuItem("Tools/Caves of Ooo/Spawn Ring 3D/Import completed Blender export")]
        ...
        {
            var args=Environment.GetCommandLineArgs();string source=Argument(args,"-spawnRing3dSource");
        ...
        public static Report Build(string sourceDirectory,string reportPath=null)
        {
            ...
            try
            {
                ...
                c.Report.modelCount=definition.models.Length;c.Report.blueprintCount=definition.blueprints.Length;
                c.Report.fellingOwnerCount=definition.fellingOwners.Length;c.Report.rendererIndex=village.RendererIndex;
                foreach(string folder in new[]{"Definitions","Models","Textures","Materials","Prefabs","Animations"})EnsureAssetFolder(Art+"/"+folder);
                EnsureAssetFolder("Assets/Resources/SpawnRing3D");TrackGuid(c,LibraryPath);
                CopyIfChanged(c,catalogFile,Art+"/Definitions/catalog.json");
                string palettePath=Art+"/Textures/SpawnRingPalette.png";
                ...
                var world=GetMaterial(c,Art+"/Materials/SpawnRingPalette.mat",village.WorldMaterial.shader);
                ...
                var water=GetMaterial(c,Art+"/Materials/SpawnRingWater.mat",village.WaterMaterial.shader);
                ...
                SaveOwned(world);SaveOwned(water);
                ...
                foreach(var model in definition.models)
                {
                    ...
                    try
                    {
                        ...
                        bindings.Add(new SpawnRing3DLibrary.ModelBinding{Id=model.id,Prefab=prefab});
                    }
                ...
                var library=AssetDatabase.LoadAssetAtPath<SpawnRing3DLibrary>(LibraryPath);
                if(library==null)
                {
                    RefuseWrongAssetType(LibraryPath);library=ScriptableObject.CreateInstance<SpawnRing3DLibrary>();
                ...
                library.WorldMaterial=world;library.WaterMaterial=water;library.CompositeMaterial=village.CompositeMaterial;
                library.Renderer=village.Renderer;library.RendererIndex=village.RendererIndex;library.EquipmentLibrary=village;library.Models=bindings.ToArray();
                ...
                Debug.Log("[SpawnRing3D] Imported "+bindings.Count+" native-ring model prefabs; report "+c.ReportPath);
                return c.Report;
            }
            catch(Exception error){c.Report.status="failed";c.Report.error=error.ToString();Debug.LogError("[SpawnRing3D] Explicit import failed: "+error.Message);throw;}
            finally
            {
        ...
        static void ConfigureModel(string path,SpawnRing3DCatalog.Model model)
        {
        ...
        static Dictionary<string,AnimationClip> ResolveClips(string path,SpawnRing3DCatalog.Model model)
        {
        ...
        static void AssignMaterials(GameObject instance,Material world,Material water,string id)
        {
            ...
            foreach(var renderer in renderers)
            {
                ...
                for(int i=0;i<source.Length;i++)
                {
                    ...
                    if(MaterialName(materialName,"SpawnRingWater"))target[i]=water;
                    else if(MaterialName(materialName,"SpawnRingPalette"))target[i]=world;
        ...
        static ModelReport ValidateImportedModel(SpawnRing3DCatalog.Model model,GameObject wrapper,string asset,string prefab,Dictionary<string,AnimationClip> clips)
        {
        ...
        {
            string path=Art+"/Textures/SpawnRingFogUnbound.asset";TrackGuid(c,path);
            var t=AssetDatabase.LoadAssetAtPath<Texture2D>(path);
            if(t==null){RefuseWrongAssetType(path);t=new Texture2D(80,25,TextureFormat.RGBA32,false,true){name="SpawnRingFogUnbound"};AssetDatabase.CreateAsset(t,path);c.Changes.Add(path);}
