...
{
    /// <summary>Native census and disposable previews for the pruning post
    /// and raised boatyard. Does not modify or save the user's active scene.</summary>
    public static class WorkshopBogCompositionPreviewBatch
    {
        ...
        {
            string output=Environment.GetEnvironmentVariable("WORKSHOP_BOG_PREVIEW_OUT");
            if(string.IsNullOrEmpty(output))throw new InvalidOperationException("WORKSHOP_BOG_PREVIEW_OUT is required.");
            ...
            foreach(string id in new[]{"Overworld.6.6.0","Overworld.15.6.0"})
        ...
        [UnityEditor.MenuItem("Caves of Ooo/Composition/Render Cinderhold and Sumphold previews")]
        public static void RenderFromMenu()
        {
            string old=Environment.GetEnvironmentVariable("WORKSHOP_BOG_PREVIEW_OUT");
            try
            {
                Environment.SetEnvironmentVariable("WORKSHOP_BOG_PREVIEW_OUT",Path.GetFullPath("Docs/Verification/VoxelWorld/WorkshopBog-"+DateTime.Now.ToString("yyyyMMdd-HHmmss")));
                Run();
            }
            finally{Environment.SetEnvironmentVariable("WORKSHOP_BOG_PREVIEW_OUT",old);}
        }
        public static void BuildKits(){CinderholdVoxelKitBuilder.Run();SumpholdVoxelKitBuilder.Run();}
        ...
        public static void Run()
        {
            ...
            try
            {
                ...
                foreach(int seed in new[]{64,1729,729490642})
                {
                    ...
                    bool cinderhold=CinderholdCompositionPlan.IsSupportedZone(id);
                    string form=cinderhold?"Cinderhold":"Sumphold";
