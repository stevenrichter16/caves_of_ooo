                ...
                foreach(var form in new[]{Formation.Hedgerow,Formation.FieldStrips,Formation.OldRoad,Formation.FlowerMeadow,Formation.Fallow,Formation.RiverMeadow})
                    foreach(int seed in new[]{64,1729,729490642})
                    {
                        ...
                        if(native)
                        {
                            ...
                            z=manager.GetZone(nativeId);character=SpreadCompositionPlan.Create(nativeId,seed).Condition;
                        }
                        watch.Stop();
                        var root=new GameObject("Temporary Spread preview");
                        ...
                        try
                        {
                            ...
                            File.WriteAllText(Path.Combine(output,form+"-"+seed+"-plan.txt"),(native?SpreadCompositionPlan.Create(z.ZoneID,seed):terrain.Plan).Signature());
                            rows.Add(new Row{formation=form.ToString(),seed=seed,character=character,image=name,zoneId=z.ZoneID,nativePipeline=native,
                                trees=Count(z,"Tree"),bushes=Count(z,"Bush"),columns=Count(z,"Hedge"),
                                compost=Count(z,"CropRow"),cache=Count(z,"FlowerField"),
