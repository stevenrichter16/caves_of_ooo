...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class GameAuditConsumablesBenchBatch
    {
        private const string Prefix = "GA02a.NativeBench.";
        static GameAuditConsumablesBenchBatch() { if (SessionState.GetBool(Prefix + "active", false)) Subscribe(); }

        [MenuItem("Caves Of Ooo/Scenarios/Items/Consumable Ownership Audit")]
        private static void Launch()
        {
            if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()) RunCore(true, false);
        }
        [MenuItem("Caves Of Ooo/Scenarios/Items/Consumable Ownership Audit", true)]
        private static bool CanLaunch() => !EditorApplication.isPlayingOrWillChangePlaymode;

        public static void Run() => RunCore(true, true);
        public static void RunWithoutBootMenu() => RunCore(false, true);
        private static void RunCore(bool showBootMenu, bool exitEditor)
        {
            string id = "GA02a-bench-" + Guid.NewGuid().ToString("N");
            ...
            SessionState.SetBool(Prefix + "preDismiss", !showBootMenu);
        ...
        private static void Apply(Zone zone, EntityFactory factory, Entity player, TurnManager turns)
        {
            ...
            new GameAuditConsumablesBench { PreDismissBootMenu = SessionState.GetBool(Prefix + "preDismiss", false) }
                .Apply(new ScenarioContext(zone, factory, player, turns, 63));
        }
        private static void Poll()
        {
            if (!SessionState.GetBool(Prefix + "active", false)) return;
            var driver = UnityEngine.Object.FindFirstObjectByType<GameAuditConsumablesBenchPlayer>();
            ...
            { Debug.LogError("[GameAuditConsumablesBench] Native Play timed out before a complete audit and capture."); Finish(2); }
        }
        ...
        private static void Finish(int code)
        {
            ...
            Debug.Log("[GameAuditConsumablesBench] Native capture exit=" + code);
