...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class GameAuditLookKeyBenchBatch
    {
        private const string Prefix = "FLOW6.NativeBench.";
        static GameAuditLookKeyBenchBatch() { if (SessionState.GetBool(Prefix + "active", false)) Subscribe(); }

        [MenuItem("Caves Of Ooo/Scenarios/Items/Look Key Audit")]
        private static void Launch()
        {
            if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()) RunCore(false);
        }
        [MenuItem("Caves Of Ooo/Scenarios/Items/Look Key Audit", true)]
        ...
        {
            string id = "FLOW6-bench-" + Guid.NewGuid().ToString("N");
        ...
        private static void Poll()
        {
            ...
            { Debug.LogError("[GameAuditLookKeyBench] Native Play timed out before a complete audit."); Finish(2); }
        }
        ...
        private static void Finish(int code)
        {
            ...
            Debug.Log("[GameAuditLookKeyBench] Native capture exit=" + code);
