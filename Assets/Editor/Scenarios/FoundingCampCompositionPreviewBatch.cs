...
{
    /// <summary>Native census and disposable previews for the founding chamber
    /// and Tent-Right hub. Does not modify or save the user's active scene.</summary>
    public static class FoundingCampCompositionPreviewBatch
    {
        ...
        {
            string output=Environment.GetEnvironmentVariable("FOUNDING_CAMP_PREVIEW_OUT");
            if(string.IsNullOrEmpty(output))throw new InvalidOperationException("FOUNDING_CAMP_PREVIEW_OUT is required.");
            ...
            foreach(string id in new[]{"Overworld.4.6.0","Overworld.4.6.1","Overworld.4.6.2","Overworld.8.16.0"})
        ...
        [UnityEditor.MenuItem("Caves of Ooo/Composition/Render Olderdeep and Wellmeet previews")]
        public static void RenderFromMenu()
        {
            string old=Environment.GetEnvironmentVariable("FOUNDING_CAMP_PREVIEW_OUT");
            try
            {
                Environment.SetEnvironmentVariable("FOUNDING_CAMP_PREVIEW_OUT",Path.GetFullPath("Docs/Verification/VoxelWorld/FoundingCamp-"+DateTime.Now.ToString("yyyyMMdd-HHmmss")));
                Run();
            }
            finally{Environment.SetEnvironmentVariable("FOUNDING_CAMP_PREVIEW_OUT",old);}
        }
        public static void BuildKits(){OlderdeepVoxelKitBuilder.Run();WellmeetVoxelKitBuilder.Run();}
        ...
        public static void Run()
        {
            ...
            try
            {
                ...
                foreach(int seed in new[]{64,1729,729490642})
                {
                    ...
                    bool olderdeep=OlderdeepCompositionPlan.IsSupportedZone(id);
                    string form=olderdeep
                        ?id.EndsWith(".0",StringComparison.Ordinal)?"ShelteredMouth":id.EndsWith(".1",StringComparison.Ordinal)?"UsedDescent":"FoundingChamber"
                        :"WellmeetCamp";
                    ...
                    try
                    {
                        ...
                        rows.Add(new Row{area=olderdeep?"Olderdeep":"Wellmeet",formation=form,zoneId=id,image=name,seed=seed,
