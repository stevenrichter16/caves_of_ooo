...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class GameAuditCraftReceiptBenchBatch
    {
        private const string Prefix = "GA02k.NativeBench.";
        static GameAuditCraftReceiptBenchBatch() { if (SessionState.GetBool(Prefix + "active", false)) Subscribe(); }

        [MenuItem("Caves Of Ooo/Scenarios/Items/Craft Receipt Audit")]
        private static void Launch()
        {
            if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()) RunCore(false);
        }
        [MenuItem("Caves Of Ooo/Scenarios/Items/Craft Receipt Audit", true)]
        ...
        {
            string id = "GA02k-bench-" + Guid.NewGuid().ToString("N");
        ...
        private static void Apply(Zone zone, EntityFactory factory, Entity player, TurnManager turns)
        {
            ...
            new GameAuditCraftReceiptBench()
                .Apply(new ScenarioContext(zone, factory, player, turns, 63));
        }
        private static void Poll()
        {
            if (!SessionState.GetBool(Prefix + "active", false)) return;
            var driver = UnityEngine.Object.FindFirstObjectByType<GameAuditCraftReceiptBenchPlayer>();
            ...
            { Debug.LogError("[GameAuditCraftReceiptBench] Native Play timed out before a complete audit."); Finish(2); }
        }
        ...
        private static void Finish(int code)
        {
            ...
            Debug.Log("[GameAuditCraftReceiptBench] Native capture exit=" + code);
