...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class GameAuditTransferBenchBatch
    {
        private const string Prefix = "GA02c.NativeBench.";
        static GameAuditTransferBenchBatch() { if (SessionState.GetBool(Prefix + "active", false)) Subscribe(); }

        [MenuItem("Caves Of Ooo/Scenarios/Items/Transfer Conservation Audit")]
        private static void Launch()
        {
            if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()) RunCore(false);
        }
        [MenuItem("Caves Of Ooo/Scenarios/Items/Transfer Conservation Audit", true)]
        ...
        {
            string id = "GA02c-bench-" + Guid.NewGuid().ToString("N");
        ...
        private static void Poll()
        {
            ...
            { Debug.LogError("[GameAuditTransferBench] Native Play timed out before a complete audit and capture."); Finish(2); }
        }
        ...
        private static void Finish(int code)
        {
            ...
            Debug.Log("[GameAuditTransferBench] Native capture exit=" + code);
