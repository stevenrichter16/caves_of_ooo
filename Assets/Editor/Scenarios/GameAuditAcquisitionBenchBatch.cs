...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class GameAuditAcquisitionBenchBatch
    {
        private const string Prefix = "GA02d.NativeBench.";
        static GameAuditAcquisitionBenchBatch() { if (SessionState.GetBool(Prefix + "active", false)) Subscribe(); }

        [MenuItem("Caves Of Ooo/Scenarios/Items/Acquisition Conservation Audit")]
        private static void Launch()
        {
            if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()) RunCore(false);
        }
        [MenuItem("Caves Of Ooo/Scenarios/Items/Acquisition Conservation Audit", true)]
        ...
        {
            string id = "GA02d-bench-" + Guid.NewGuid().ToString("N");
        ...
        private static void Apply(Zone zone, EntityFactory factory, Entity player, TurnManager turns)
        {
            ...
            new GameAuditAcquisitionBench()
                .Apply(new ScenarioContext(zone, factory, player, turns, 63));
        }
        private static void Poll()
        {
            if (!SessionState.GetBool(Prefix + "active", false)) return;
            var driver = UnityEngine.Object.FindFirstObjectByType<GameAuditAcquisitionBenchPlayer>();
            ...
            { Debug.LogError("[GameAuditAcquisitionBench] Native Play timed out before a complete audit and capture."); Finish(2); }
        }
        ...
        private static void Finish(int code)
        {
            ...
            Debug.Log("[GameAuditAcquisitionBench] Native capture exit=" + code);
