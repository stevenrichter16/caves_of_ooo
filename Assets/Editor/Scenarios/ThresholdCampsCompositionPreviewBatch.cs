...
{
    /// <summary>Native census and disposable previews for the oath gathering
    /// and frontier post. Does not modify or save the user's active scene.</summary>
    public static class ThresholdCampsCompositionPreviewBatch
    {
        ...
        {
            string output=Environment.GetEnvironmentVariable("THRESHOLD_CAMPS_PREVIEW_OUT");
            if(string.IsNullOrEmpty(output))throw new InvalidOperationException("THRESHOLD_CAMPS_PREVIEW_OUT is required.");
            ...
            foreach(string id in new[]{"Overworld.5.17.0","Overworld.18.18.0"})
            foreach(int seed in new[]{64,1729,729490642,1})
            {
        ...
        [UnityEditor.MenuItem("Caves of Ooo/Composition/Render First Tent and Last Counter previews")]
        public static void RenderFromMenu()
        {
            string old=Environment.GetEnvironmentVariable("THRESHOLD_CAMPS_PREVIEW_OUT");
            try
            {
                Environment.SetEnvironmentVariable("THRESHOLD_CAMPS_PREVIEW_OUT",Path.GetFullPath("Docs/Verification/VoxelWorld/ThresholdCamps-"+DateTime.Now.ToString("yyyyMMdd-HHmmss")));
                Run();
            }
            finally{Environment.SetEnvironmentVariable("THRESHOLD_CAMPS_PREVIEW_OUT",old);}
        }
        public static void BuildKits(){FirstTentVoxelKitBuilder.Run();LastCounterVoxelKitBuilder.Run();}
        ...
        public static void Run()
        {
            ...
            try
            {
                ...
                foreach(int seed in new[]{64,1729,729490642,1})
                {
                    ...
                    bool ledger=FirstTentCompositionPlan.IsSupportedZone(id);
                    string form=ledger?"FirstTent":"LastCounter";
                    ...
                    try
                    {
                        ...
                        rows.Add(new Row{area=form,formation=ledger?FirstTentCompositionPlan.Create(id,seed).FormationName:LastCounterCompositionPlan.Create(id,seed).FormationName,zoneId=id,image=name,seed=seed,
