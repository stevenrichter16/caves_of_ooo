...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class GameAuditCraftAvailabilityBenchBatch
    {
        private const string Prefix = "GA02j.NativeBench.";
        static GameAuditCraftAvailabilityBenchBatch() { if (SessionState.GetBool(Prefix + "active", false)) Subscribe(); }

        [MenuItem("Caves Of Ooo/Scenarios/Items/Craft Availability Audit")]
        private static void Launch()
        {
            if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()) RunCore(false);
        }
        [MenuItem("Caves Of Ooo/Scenarios/Items/Craft Availability Audit", true)]
        ...
        {
            string id = "GA02j-bench-" + Guid.NewGuid().ToString("N");
        ...
        private static void Apply(Zone zone, EntityFactory factory, Entity player, TurnManager turns)
        {
            ...
            new GameAuditCraftAvailabilityBench()
                .Apply(new ScenarioContext(zone, factory, player, turns, 63));
        }
        private static void Poll()
        {
            if (!SessionState.GetBool(Prefix + "active", false)) return;
            var driver = UnityEngine.Object.FindFirstObjectByType<GameAuditCraftAvailabilityBenchPlayer>();
            ...
            { Debug.LogError("[GameAuditCraftAvailabilityBench] Native Play timed out before a complete audit."); Finish(2); }
        }
        ...
        private static void Finish(int code)
        {
            ...
            Debug.Log("[GameAuditCraftAvailabilityBench] Native capture exit=" + code);
