...
{
    [InitializeOnLoad]
    public static class GameAuditDecodeSaveBenchBatch
    {
        private const string Prefix="GA03d.NativeBench.";
        static GameAuditDecodeSaveBenchBatch(){if(SessionState.GetBool(Prefix+"active",false))Subscribe();}
        public static void Run()=>RunMode("new");
        [MenuItem("Caves Of Ooo/Scenarios/Save/Corrupt Save Recovery Audit")]
        private static void Launch(){if(EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo())RunMode("new",false);}
        [MenuItem("Caves Of Ooo/Scenarios/Save/Corrupt Save Recovery Audit",true)]
        ...
        {
            string marker="GA03d-marker-"+Guid.NewGuid().ToString("N");
            ...
            catch(Exception ex){Debug.LogError("[GameAuditDecodeSaveBench] Launch failed: "+ex);Finish(4);}
        }
        ...
        private static void Apply(Zone zone,EntityFactory factory,Entity player,TurnManager turns)
        {
            ...
            var driver=new GameObject("Corrupt save recovery audit").AddComponent<GameAuditDecodeSaveBenchPlayer>();
            driver.Initialize(player);
        }
        private static void Poll()
        {
            if(!SessionState.GetBool(Prefix+"active",false))return;
            var driver=UnityEngine.Object.FindFirstObjectByType<GameAuditDecodeSaveBenchPlayer>();
            if(driver!=null&&driver.Finished){Finish(driver.Failures+SessionState.GetInt(Prefix+"errors",0)==0?0:1);return;}
            if(EditorApplication.timeSinceStartup>SessionState.GetFloat(Prefix+"deadline",0)){Debug.LogError("[GameAuditDecodeSaveBench] Timed out.");Finish(2);}
        }
        private static void OnLog(string message,string stack,LogType type)
        {
            var driver=UnityEngine.Object.FindFirstObjectByType<GameAuditDecodeSaveBenchPlayer>();
            if(driver!=null&&driver.ObserveExpectedRejection(message,type))return;
        ...
        private static void Finish(int code)
        {
            ...
            SaveGameService.RegisterRuntime(null,null);Debug.Log("[GameAuditDecodeSaveBench] Native capture exit="+code);
