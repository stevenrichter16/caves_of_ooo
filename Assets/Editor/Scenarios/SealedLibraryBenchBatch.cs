...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class SealedLibraryBenchBatch
    {
        private const string Prefix = "W66.NativeBench.";
        static SealedLibraryBenchBatch() { if (SessionState.GetBool(Prefix + "active", false)) Subscribe(); }
        ...
        private static void Apply(Zone zone, EntityFactory factory, Entity player, TurnManager turns)
        {
            ...
            new SealedLibraryBench().Apply(new ScenarioContext(zone, factory, player, turns, 66));
        }
        private static void Poll()
        {
            if (!SessionState.GetBool(Prefix + "active", false)) return;
            var driver = UnityEngine.Object.FindFirstObjectByType<SealedLibraryBenchPlayer>();
            ...
            { Debug.LogError("[SealedLibraryBench] Native Play timed out before a complete audit and capture."); Finish(2); }
        }
