...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class VoxelWorldNativeAuditBatch
    {
        private const string Prefix="VoxelWorld.NativeAudit.";
        private const int AuditWorldSeed=729490642;
        private const BindingFlags All=BindingFlags.Public|BindingFlags.NonPublic|BindingFlags.Instance|BindingFlags.Static;
        static VoxelWorldNativeAuditBatch()
        {
        ...
        [MenuItem("Caves Of Ooo/Scenarios/World/Voxel World Acceptance")]
        ...
        private static void RunCore(bool exitEditor)
        {
            ...
            {
                Configure1080p();
                string token=NativeSaveIsolation.Begin(Prefix,"VoxelWorld-native-marker-"+Guid.NewGuid().ToString("N"),false);
            ...
            {
                Debug.LogError("[VoxelWorldNativeAudit] Launch failed: "+error);
        ...
        {GameBootstrap.OnAfterBootstrap-=Apply;new GameObject("Multi-Cell Ridge Native Acceptance").AddComponent<VoxelWorldNativeAudit>().Initialize(SessionState.GetString(Prefix+"runId",""));}
        private static void Poll()
        {
            if(!SessionState.GetBool(Prefix+"active",false)||SessionState.GetBool(Prefix+"finishing",false))return;
            var driver=UnityEngine.Object.FindFirstObjectByType<VoxelWorldNativeAudit>();
        ...
        private static void Finish(int code)
        {
            ...
            SaveGameService.RegisterRuntime(null,null);Debug.Log("[VoxelWorldNativeAudit] Native capture exit="+code);
        ...
        private static void AfterNativeCleanup()
        {
            ...
            string dir=Path.GetFullPath(Path.Combine(Application.dataPath,"../Docs/Verification/VoxelWorld"));Directory.CreateDirectory(dir);
            string native=Path.Combine(dir,"VWN-"+runId+"-native.json");
            try
            {
            ...
            receipt.exitCode=code;File.WriteAllText(Path.Combine(dir,"VWN-"+runId+"-cleanup.json"),JsonUtility.ToJson(receipt,true));
            SessionState.SetBool(Prefix+"active",false);SessionState.SetBool(Prefix+"finishing",false);Debug.Log("[VoxelWorldNativeAudit] Scene/view/private-save cleanup exit="+code);
        ...
        private static bool ValidateReceiptMetadata(string json,string expectedRunId,string privateRoot)
        {
            ...
            try
            {
                ...
                if(f.region==null||f.region.Length!=4||f.borders==null||f.borders.Length!=6)return false;
                var zones=new HashSet<string>(new[]{"Overworld.2.6.0","Overworld.3.6.0","Overworld.3.7.0","Overworld.4.7.0"});
                foreach(var z in f.region)if(z==null||!zones.Remove(z.zoneId)||!z.active||z.swapped<=0||z.missing!=0||z.fullReveal||!z.playerVisible||z.visible<=0||z.visible>=2000)return false;
                if(zones.Count!=0)return false;
                var edges=new HashSet<string>(new[]{"Overworld.2.6.0>Overworld.3.6.0","Overworld.3.6.0>Overworld.2.6.0", "Overworld.3.6.0>Overworld.3.7.0","Overworld.3.7.0>Overworld.3.6.0", "Overworld.3.7.0>Overworld.4.7.0","Overworld.4.7.0>Overworld.3.7.0"});
                foreach(var b in f.borders)if(b==null||!b.pass||!edges.Remove(b.from+">"+b.to)||b.tickAfter<=b.tickBefore)return false;
        ...
        private static bool ValidateFinalReportForRun(string json,string expectedRunId,string privateRoot)
        {
            ...
            try
            {
                ...
                string directory=Path.GetFullPath(Path.Combine(Application.dataPath,"../Docs/Verification/VoxelWorld"));
                string stem="VWN-"+expectedRunId;
                if(f.screenshots==null||f.screenshots.Length!=7||f.screenshots.Distinct(StringComparer.Ordinal).Count()!=7)return false;
                foreach(var label in new[]{"south-entry","destructible-ridge","restored-save","spawn-town","western-compost-field","southern-ridge","eastern-grove"})
                {
        ...
        [Serializable] private sealed class FinalNative{public VoxelWorldNativeAudit.RegionRow[] region;public VoxelWorldNativeAudit.BorderRow[] borders;public VoxelWorldNativeAudit.ProfileReport profile;public string runId,saveRoot;public int failures,unexpectedErrors,worldSeed,screenWidth,screenHeight,nativeSteps;
            public VoxelWorldNativeAudit.CheckRow[] checks;public string[] screenshots;
