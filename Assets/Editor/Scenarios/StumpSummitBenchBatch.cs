...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class StumpSummitBenchBatch
    {
        private const string Prefix = "W63b.NativeBench.";
        static StumpSummitBenchBatch() { if (SessionState.GetBool(Prefix + "active", false)) Subscribe(); }

        public static void Run()
        {
            string id = "W63b-bench-" + Guid.NewGuid().ToString("N");
        ...
        private static void Apply(Zone zone, EntityFactory factory, Entity player, TurnManager turns)
        {
            ...
            new StumpSummitBench().Apply(new ScenarioContext(zone, factory, player, turns, 63));
        }
        private static void Poll()
        {
            if (!SessionState.GetBool(Prefix + "active", false)) return;
            var driver = UnityEngine.Object.FindFirstObjectByType<StumpSummitBenchPlayer>();
            ...
            { Debug.LogError("[StumpSummitBench] Native Play timed out before a complete audit and capture."); Finish(2); }
        }
        ...
        private static void Finish(int code)
        {
            ...
            Debug.Log("[StumpSummitBench] Native capture exit=" + code);
