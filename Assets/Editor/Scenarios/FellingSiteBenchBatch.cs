...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class FellingSiteBenchBatch
    {
        private const string Prefix = "W65.NativeBench.";
        static FellingSiteBenchBatch() { if (SessionState.GetBool(Prefix + "active", false)) Subscribe(); }

        public static void Run()
        {
            string id = "W65-bench-" + Guid.NewGuid().ToString("N");
        ...
        private static void Apply(Zone zone, EntityFactory factory, Entity player, TurnManager turns)
        {
            ...
            new FellingSiteBench().Apply(new ScenarioContext(zone, factory, player, turns, 65));
        }
        private static void Poll()
        {
            if (!SessionState.GetBool(Prefix + "active", false)) return;
            var driver = UnityEngine.Object.FindFirstObjectByType<FellingSiteBenchPlayer>();
            ...
            { Debug.LogError("[FellingSiteBench] Native Play timed out before a complete audit and capture."); Finish(2); }
        }
        ...
        private static void Finish(int code)
        {
            ...
            Debug.Log("[FellingSiteBench] Native capture exit=" + code);
