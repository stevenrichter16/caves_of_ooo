...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class GameAuditSplitIdentityBenchBatch
    {
        private const string Prefix = "FLOW4.NativeBench.";
        static GameAuditSplitIdentityBenchBatch() { if (SessionState.GetBool(Prefix + "active", false)) Subscribe(); }

        [MenuItem("Caves Of Ooo/Scenarios/Items/Split Identity Audit")]
        private static void Launch()
        {
            if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()) RunCore(false);
        }
        [MenuItem("Caves Of Ooo/Scenarios/Items/Split Identity Audit", true)]
        ...
        {
            string id = "FLOW4-bench-" + Guid.NewGuid().ToString("N");
        ...
        private static void Apply(Zone zone, EntityFactory factory, Entity player, TurnManager turns)
        {
            ...
            new GameAuditSplitIdentityBench()
                .Apply(new ScenarioContext(zone, factory, player, turns, 63));
        }
        private static void Poll()
        {
            if (!SessionState.GetBool(Prefix + "active", false)) return;
            var driver = UnityEngine.Object.FindFirstObjectByType<GameAuditSplitIdentityBenchPlayer>();
            ...
            { Debug.LogError("[GameAuditSplitIdentityBench] Native Play timed out before a complete audit."); Finish(2); }
        }
        ...
        private static void Finish(int code)
        {
            ...
            Debug.Log("[GameAuditSplitIdentityBench] Native capture exit=" + code);
