...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class GameAuditHandoverBenchBatch
    {
        private const string Prefix = "GA02b.NativeBench.";
        static GameAuditHandoverBenchBatch() { if (SessionState.GetBool(Prefix + "active", false)) Subscribe(); }

        [MenuItem("Caves Of Ooo/Scenarios/Items/Handover and Repair Audit")]
        private static void Launch()
        {
            if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()) RunCore(false);
        }
        [MenuItem("Caves Of Ooo/Scenarios/Items/Handover and Repair Audit", true)]
        ...
        {
            string id = "GA02b-bench-" + Guid.NewGuid().ToString("N");
        ...
        private static void Poll()
        {
            ...
            { Debug.LogError("[GameAuditHandoverBench] Native Play timed out before a complete audit and capture."); Finish(2); }
        }
        ...
        private static void Finish(int code)
        {
            ...
            Debug.Log("[GameAuditHandoverBench] Native capture exit=" + code);
