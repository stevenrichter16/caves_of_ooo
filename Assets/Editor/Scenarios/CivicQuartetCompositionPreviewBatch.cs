...
{
    /// <summary>Native census and disposable previews for Gantry, Tine,
    /// Quillhold and Tally. Does not modify or save the user's active scene.</summary>
    public static class CivicQuartetCompositionPreviewBatch
    {
        ...
        [Serializable] public sealed class CensusRow
        {
            ...
            public int seed,actualWorldSeed,entities;
        ...
        {
            string output=Environment.GetEnvironmentVariable("CIVIC_QUARTET_PREVIEW_OUT");
            if(string.IsNullOrEmpty(output))throw new InvalidOperationException("CIVIC_QUARTET_PREVIEW_OUT is required.");
            ...
            foreach(string id in new[]{"Overworld.7.8.0","Overworld.13.7.0","Overworld.14.9.0","Overworld.10.14.0"})
            foreach(int seed in Seeds(id))
            {
                var manager=OverworldZoneManager.CreateDetached(factory,seed);
                VerifySeed(manager,seed);var zone=manager.GetZone(id);
                var at=WorldMap.FromZoneID(id);
                rows.Add(new CensusRow{zoneId=id,seed=seed,actualWorldSeed=manager.WorldSeed,entities=zone.EntityCount,
        ...
        [Serializable] public sealed class Row
        {
            ...
            public int seed,actualWorldSeed,entities,solids,water,creatures,meshes,missing,unmodeled;
        ...
        [UnityEditor.MenuItem("Caves of Ooo/Composition/Render Gantry, Tine, Quillhold and Tally previews")]
        public static void RenderFromMenu()
        {
            string old=Environment.GetEnvironmentVariable("CIVIC_QUARTET_PREVIEW_OUT");
            try
            {
                Environment.SetEnvironmentVariable("CIVIC_QUARTET_PREVIEW_OUT",Path.GetFullPath("Docs/Verification/VoxelWorld/CivicQuartet-"+DateTime.Now.ToString("yyyyMMdd-HHmmss")));
                Run();
            }
            finally{Environment.SetEnvironmentVariable("CIVIC_QUARTET_PREVIEW_OUT",old);}
        }
        public static void BuildKits(){GantryVoxelKitBuilder.Run();TineVoxelKitBuilder.Run();QuillholdVoxelKitBuilder.Run();TallyVoxelKitBuilder.Run();}
        ...
        public static void Run()
        {
            ...
            try
            {
                ...
                foreach(int seed in Seeds(id))
                {
                    ...
                    string form=id==GantryCompositionPlan.ZoneID?"Gantry":id==TineCompositionPlan.ZoneID?"Tine":id==QuillholdCompositionPlan.ZoneID?"Quillhold":"Tally";
                    ...
                    try
                    {
                        ...
                        rows.Add(new Row{area=form,formation=Formation(id,seed),zoneId=id,image=name,seed=seed,actualWorldSeed=manager.WorldSeed,
        ...
        private static void VerifySeed(OverworldZoneManager manager,int seed)
        {
            if(manager.WorldSeed!=seed)
                throw new InvalidOperationException("Preview seed changed during native generation: "+seed+" -> "+manager.WorldSeed);
        }
        private static string Formation(string id,int seed)
        {return id==GantryCompositionPlan.ZoneID?GantryCompositionPlan.Create(id,seed).FormationName:id==TineCompositionPlan.ZoneID?TineCompositionPlan.Create(id,seed).FormationName:id==QuillholdCompositionPlan.ZoneID?QuillholdCompositionPlan.Create(id,seed).FormationName:TallyCompositionPlan.Create(id,seed).FormationName;}
        private static IEnumerable<int> Seeds(string id)
        {
            var names=new HashSet<string>();
            foreach(int seed in new[]{64,1729,729490642}.Concat(Enumerable.Range(1,64)))
                if(names.Add(Formation(id,seed))){yield return seed;if(names.Count==3)yield break;}
            throw new InvalidOperationException("Three distinct semantic formations required: "+id);
        }
