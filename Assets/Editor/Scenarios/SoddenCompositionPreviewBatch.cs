...
namespace CavesOfOoo.Editor
{
    ...
    public static class SoddenCompositionPreviewBatch
    {
        [Serializable] public sealed class Row
        {
            public string formation,character,image,zoneId;
            public bool nativePipeline;
            public int seed,snags,reeds,banks,pools,boards,bodies,toads,meshes,missing;
            public double generationMilliseconds;
        }
        [Serializable] public sealed class Report { public Row[] rows; }
        public static void BuildAndRun(){SoddenVoxelKitBuilder.Run();Run();}
        public static void Run()
        {
            string output=Environment.GetEnvironmentVariable("SODDEN_PREVIEW_OUT");
            if(string.IsNullOrEmpty(output))throw new InvalidOperationException("SODDEN_PREVIEW_OUT required.");
            ...
            bool native=Environment.GetEnvironmentVariable("SODDEN_PREVIEW_NATIVE")=="1";
            try
            {
                foreach(var form in new[]{Formation.OpenMire,Formation.PeatCuts,Formation.ReedMaze,Formation.DrownedCopse,Formation.Causeway,Formation.BogFace})
                    foreach(int seed in new[]{64,1729,729490642})
                    {
                        var watch=System.Diagnostics.Stopwatch.StartNew();
                        var z=new Zone("Overworld.16.3.0");
                        var terrain=new SoddenCompositionBuilder(seed){FormationOverride=form};
                        ...
                        string character=terrain.Plan.Condition;
                        if(native)
                        {
                            ...
                            for(int x=0;x<20&&nativeId==null;x++)for(int y=0;y<20&&nativeId==null;y++)
                            {
                                ...
                                if(SoddenCompositionPlan.IsWildernessZone(id)&&manager.WorldMap.GetPOI(x,y)==null
                                    &&FormationSelector.For(BiomeType.Sodden,id)==form)nativeId=id;
                            }
                            if(nativeId==null)throw new Exception("No native formation example.");
                            z=manager.GetZone(nativeId);character=SoddenCompositionPlan.Create(nativeId,seed).Condition;
                        }
                        watch.Stop();
                        var root=new GameObject("Temporary Sodden preview");
                        ...
                        try
                        {
                            ...
                            File.WriteAllText(Path.Combine(output,form+"-"+seed+"-plan.txt"),(native?SoddenCompositionPlan.Create(z.ZoneID,seed):terrain.Plan).Signature());
                            rows.Add(new Row{formation=form.ToString(),seed=seed,character=character,image=name,zoneId=z.ZoneID,nativePipeline=native,
                                snags=Count(z,"DeadTree"),reeds=Count(z,"Reeds"),banks=Count(z,"PeatBank"),
                                pools=Count(z,"MirePool"),boards=Count(z,"Duckboard"),bodies=Count(z,"BogTakenBody"),toads=Count(z,"MawToad"),
