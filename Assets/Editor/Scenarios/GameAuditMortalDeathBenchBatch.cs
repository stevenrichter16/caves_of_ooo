...
namespace CavesOfOoo.Editor
{
    ...
    [InitializeOnLoad]
    public static class GameAuditMortalDeathBenchBatch
    {
        private const string Prefix = "GA03h.NativeBench.";
        static GameAuditMortalDeathBenchBatch() { if (SessionState.GetBool(Prefix + "active", false)) Subscribe(); }
        ...
        [MenuItem("Caves Of Ooo/Scenarios/Combat/Mortal Death Audit")]
        private static void Launch() { if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()) RunCore(false); }
        [MenuItem("Caves Of Ooo/Scenarios/Combat/Mortal Death Audit", true)]
        ...
        {
            string marker = "GA03h-marker-" + Guid.NewGuid().ToString("N");
            ...
            catch (Exception ex) { Debug.LogError("[GameAuditMortalDeathBench] Launch failed: " + ex); Finish(4); }
        }
        ...
        private static void Apply(Zone zone, EntityFactory factory, Entity player, TurnManager turns)
        {
            ...
            new GameAuditMortalDeathBench().Apply(new ScenarioContext(zone, factory, player, turns, 67));
            string mode = SessionState.GetString(Prefix + "performanceMode", "");
            if (mode.Length > 0)
            {
                var driver = UnityEngine.Object.FindFirstObjectByType<GameAuditMortalDeathBenchPlayer>();
                if (driver == null) throw new InvalidOperationException("Missing performance driver.");
        ...
        private static void Poll()
        {
            ...
            if (EditorApplication.timeSinceStartup > SessionState.GetFloat(Prefix + "deadline", 0)) { Debug.LogError("[GameAuditMortalDeathBench] Native audit timed out."); Finish(2); }
        }
        ...
        private static void Finish(int code)
        {
            ...
            SaveGameService.RegisterRuntime(null, null); Debug.Log("[GameAuditMortalDeathBench] Native capture exit=" + code);
