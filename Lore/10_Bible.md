...
- **The binding was an overwriting** (Phase 12): the Tree did not create the world — it scraped one and wrote over it. Naming is inscription; the bound world is the over-text; the under-layer is **not blank** — *a page scraped often enough looks blank; it isn't.* Where the binding thins, the old letters bleed through: **the under-text, the palimpsest** (`TERMS.md`; the bleed phenomena, the relics, the Overwrit). No in-game source may map, date, count, or systematize the layers (Mystery Ledger, amended policy + §9).
- **The Unsaying is scraping** (Phase 12): an Unsaid person or place is not destroyed but scraped — faint, under-textual, partially recoverable by underreading at rare, lossy, morally loud cost (recovery of persons is deferred content; `Lore/Design/UNDERREADING.md`). The *sari... sari...* now carries **two readings, both live, never adjudicated**: a god failing to be born (Phase 1), and a stylus on a scraped page — the under-layer trying to write itself back (Phase 12). They may be the same event described at two distances.
- **The First Account** (Phase 12): Maeleth's original chronicle of the Felling — the oldest document in the world, bearing the only two erasures the god of Memory has ever performed: the scraped line where Naro's spoken refusal should stand, and the three scraped words at the Tree's falling moment. **Design-canon rule (the type specimen of the amended ledger policy): its recovered under-text yields ambiguity at equal-or-higher resolution, never adjudication** — the recovered words support all three Naro readings, and no source ever grades them. (`Lore/Design/FIRST-ACCOUNT.md`)
