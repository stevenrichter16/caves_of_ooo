...
public class RepairSifrah : TinkeringSifrah
{
	...
	{
		Description = "Repairing " + ContextObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutTitles: false, Short: false);
		...
		int num3 = GetTinkeringBonusEvent.GetFor(The.Player, ContextObject, "Repair", MaxTurns, 0, ref SecondaryBonus, ref Interrupt, ref PsychometryApplied, Interruptable: true, ForSifrah: true);
		if (Interrupt)
		{
			Finished = true;
			Abort = true;
		...
		{
			Popup.ShowFail("You have no usable options to employ for repairing " + ContextObject.t(int.MaxValue, null, null, AsIfKnown: false, Single: true) + ", giving you no chance of success. You can remedy this situation by improving your Intelligence and tinkering skills, or by obtaining items useful for tinkering.");
			Finished = true;
			Abort = true;
			return;
		}
		...
		else
		{
			...
			int num7 = Tokens.Count + num5 / 2 + num6 + gameCount / 20;
			slots2 = Slots.Count;
			tokens = Tokens.Count;
			asMaster = true;
			...
			if (gameCount2 + gameCount3 / 50 >= num7)
			{
				AsMaster = true;
			}
		}
