...
[Serializable]
public class BuildingTemplate : IComposite
{
	public int Width;
	...
	public BuildingTemplate(int _Width, int _Height, BuildingTemplateTile defaultTile = BuildingTemplateTile.Void)
	{
		Width = _Width;
		Height = _Height;
	...
	public void Write(SerializationWriter Writer)
	{
		...
		for (int i = 0; i < Width; i++)
		{
			...
			{
				Writer.WriteOptimized((int)Map[i, j]);
				Writer.WriteOptimized(Rooms[i, j]);
			}
		}
		Writer.WriteComposite(RoomList);
	}
	...
	public void Read(SerializationReader Reader)
	{
		...
		for (int i = 0; i < Width; i++)
		{
			for (int j = 0; j < Height; j++)
			{
				Map[i, j] = (BuildingTemplateTile)Reader.ReadOptimizedInt32();
				Rooms[i, j] = Reader.ReadOptimizedInt32();
			}
		}
		RoomList = Reader.ReadCompositeList<RoomData>();
	}

	public void AddMap(int StartX, int StartY, BuildingTemplate Source)
	{
	...
	public BuildingTemplate(int _Width, int _Height, int nSquares, bool FullSquare)
	{
		Width = _Width;
		Height = _Height;
		Map = new BuildingTemplateTile[Width, Height];
		...
		for (int i = 0; i < Height; i++)
		{
			...
			{
				Map[j, i] = BuildingTemplateTile.Void;
				Rooms[j, i] = 0;
			}
		}
		if (FullSquare)
		...
		else
		{
			...
			{
				int num = 0;
				int num2 = Stat.Random(1, _Width / 2);
				int num3 = Stat.Random(num2 + 3, _Width - 2);
				int num4 = Stat.Random(1, _Height / 2);
				num = Stat.Random(num4 + 3, _Height - 2);
		...
		for (int num15 = 0; num15 < Height; num15++)
		{
			...
			{
				if (Map[num16, num15] == BuildingTemplateTile.Inside)
				{
					FillRoom(num16, num15);
					break;
				}
			}
		}
		...
		for (int i = 0; i < Height; i++)
		{
			for (int j = 0; j < Width; j++)
			{
				if (Data != null && Data.Room[j, i] > 0)
				{
					...
					Popup._ScreenBuffer.Write((short)(49 + Data.Room[j, i]));
				}
				if (Map[j, i] == BuildingTemplateTile.Inside)
				{
					Popup._ScreenBuffer.Goto(j, i);
					Popup._ScreenBuffer.Write(58);
				}
				if (Map[j, i] == BuildingTemplateTile.Void)
				{
					Popup._ScreenBuffer.Goto(j, i);
					Popup._ScreenBuffer.Write(88);
				}
				if (Map[j, i] == BuildingTemplateTile.Outside)
				{
					Popup._ScreenBuffer.Goto(j, i);
					Popup._ScreenBuffer.Write(44);
				}
				if (Map[j, i] == BuildingTemplateTile.OutsideWall)
				{
					Popup._ScreenBuffer.Goto(j, i);
					Popup._ScreenBuffer.Write(35);
				}
				if (Map[j, i] == BuildingTemplateTile.Wall)
				{
					Popup._ScreenBuffer.Goto(j, i);
					Popup._ScreenBuffer.Write(42);
				}
				if (Map[j, i] == BuildingTemplateTile.Door)
				{
					Popup._ScreenBuffer.Goto(j, i);
				...
				{
					Popup._ScreenBuffer.Goto(j, i);
					Popup._ScreenBuffer.Write((short)(65 + Rooms[j, i]));
				}
			}
