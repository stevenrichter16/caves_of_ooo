...
public class OverlandAlgaeLake
{
	...
	public bool[,] GetWatermap(int Wx, int Wy)
	{
		...
		for (int k = -1; k <= 1; k++)
		{
			for (int l = -1; l <= 1; l++)
			{
				if (Wy + k >= 0)
				{
					array2[l + 1, k + 1] = zone.GetCell(Wx + l, Wy + k)?.HasObjectWithPropertyOrTagEqualToValue("Terrain", "LakeHinnom") ?? false;
				}
				else
				{
					array2[l + 1, k + 1] = true;
	...
	public bool BuildZone(Zone Z)
	{
				...
				for (int num = 0; num < 3; num++)
				{
					for (int num2 = 0; num2 < 3; num2++)
					{
						...
						for (int num7 = num3; num7 <= num4; num7++)
						{
							...
							{
								Z.GetCell(num7, num8).AddObject("AlgalWaterDeepPool");
							}
						}
					}
				}
		...
		for (int num11 = 0; num11 < Z.Height; num11++)
		{
			for (int num12 = 0; num12 < Z.Width; num12++)
			{
				...
				{
					Z.GetCell(num12, num11).AddObject("AlgalWaterDeepPool");
				}
			}
		}
