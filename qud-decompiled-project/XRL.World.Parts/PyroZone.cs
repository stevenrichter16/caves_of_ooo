...
public class PyroZone : IPart
{
	...
	public string StartMessage = "The air =subject.generalDirectionIfAny= starts to shimmer with heat!";

	public string StopMessage = "The air =subject.generalDirectionIfAny= ceases shimmering with heat.";
	...
	public override bool HandleEvent(EndTurnEvent E)
	{
		...
		if (Duration > 0)
		{
			DieRoll cachedDieRoll = (Level + "d" + Pyrokinesis.GetDamageDieSize(Turn)).GetCachedDieRoll();
			...
			for (int i = 0; i < activeZone.Height; i++)
			{
				for (int j = 0; j < activeZone.Width; j++)
				{
					...
					for (int count = cell.Objects.Count; k < count; k++)
					{
						...
						if (gameObject.Blueprint == ParentObject.Blueprint && gameObject.TryGetPart<PyroZone>(out var Part) && Part.GroupID == GroupID && gameObject.IsVisible())
						{
							return true;
						}
					}
			...
			if (activeZone != null)
			{
				for (int i = 0; i < activeZone.Height; i++)
				{
					for (int j = 0; j < activeZone.Width; j++)
					{
						...
						{
							GameObject gameObject = cell.Objects[k];
							if (gameObject.Blueprint == ParentObject.Blueprint && gameObject.TryGetPart<PyroZone>(out var Part) && Part.GroupID == GroupID && gameObject.InSameCellAs(Object))
							{
								return true;
							}
						}
		...
		foreach (Zone value in The.ZoneManager.CachedZones.Values)
		{
			for (int i = 0; i < value.Height; i++)
			{
				for (int j = 0; j < value.Width; j++)
				{
					...
					{
						GameObject gameObject = cell.Objects[k];
						if (gameObject.Blueprint == ParentObject.Blueprint && gameObject.TryGetPart<PyroZone>(out var Part) && Part.GroupID == GroupID)
						{
							list.Add(gameObject);
						}
					}
				...
				for (int i = 0; i < currentZone.Height; i++)
				{
					for (int j = 0; j < currentZone.Width; j++)
					{
						...
						for (int count = cell.Objects.Count; k < count; k++)
						{
							...
							if (gameObject2.Blueprint == ParentObject.Blueprint && gameObject2.TryGetPart<PyroZone>(out var Part) && Part.GroupID == GroupID && gameObject2.IsVisible())
							{
		...
		if (GameObject.Validate(ref Object))
		{
			...
			if (cell != null)
			{
				...
				for (int count = cell.Objects.Count; i < count; i++)
				{
					...
					if (gameObject.Blueprint == ParentObject.Blueprint && gameObject.TryGetPart<PyroZone>(out var Part) && Part.GroupID == GroupID && IComponent<GameObject>.Visible(gameObject))
					{
						return gameObject;
					}
				}
		...
		foreach (Zone value in The.ZoneManager.CachedZones.Values)
		{
			for (int i = 0; i < value.Height; i++)
			{
				for (int j = 0; j < value.Width; j++)
				{
					...
					{
						GameObject gameObject = cell.Objects[k];
						if (gameObject.Blueprint == ParentObject.Blueprint && gameObject.TryGetPart<PyroZone>(out var Part) && Part.GroupID == GroupID && Part.Control)
						{
							return gameObject;
						}
					}
