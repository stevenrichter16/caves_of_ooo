#!/bin/bash
# Owned import gate: strict model validation and protected scene/source/GUID receipts.
# Run this script in the background; all evidence uses a new unique directory.
set -euo pipefail
repo=/Users/steven/caves-of-ooo
source_root=${1:-$repo/ArtSource/SpawnRing3D}
...
cp "$audit_tools/import.sh" "$audit_tools/import_receipt.py" "$run_dir/"
cp "$wrapper" "$run_dir/SpawnRing3DImportAudit.cs"
python3 "$audit_tools/import_receipt.py" snapshot "$repo" "$source_root" "$run_dir/before.json"
python3 "$audit_tools/import_receipt.py" preflight "$run_dir/before.json"
...
python3 "$audit_tools/import_receipt.py" snapshot "$repo" "$source_root" "$run_dir/after.json"
if [ "$cs_errors" -ne 0 ]; then echo 'Compile errors: reports are not accepted.' >&2; exit 1; fi
python3 "$audit_tools/import_receipt.py" check "$run_dir" "$unity_exit"
